...
Current Full Firedancer Tile Flow (src/app/firedancer/topology.c)
  ...
  BAM scheduler gRPC/TLS feed --> bam tile --bam_verif--> verify --verify_dedup--> dedup --dedup_resolv--> resolv --resolv_pack--> pack --pack_execle--> execle --execle_poh--> poh
  ...
  BAM feedback/control link roles
    ...
    bank_bam: Bank tiles (fdctl/Frankendancer) and execle tiles (full Firedancer) publish
      executed `fd_bam_bundle_result_t` outcomes. Verify marks BAM parse/signature failures
      as `preprocess_failed`; dedup and resolv propagate them to pack, which publishes the
      terminal result on pack_bam_res. BAM merges bank_bam and pack_bam_res into the same
      durable FIFO result stream.
