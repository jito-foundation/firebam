...
- bam git commit `ed2c72c1a3a4968dfccee042fd45e341cdb66372`
...
- A client IP recently disconnected for high RTT may receive `failed_precondition` until the configured reconnect
  backoff expires (default 5 minutes).
...
- Missing or mismatched `Pong` does not itself disconnect the stream. The disconnect policy uses network-canary mean
  RTT when available; stream ping/pong is telemetry-only.
...
- Network-canary mean RTT above the configured threshold at a periodic policy check, subject to a near-leader
  exception.
...
- Outer grouping varies by scheduler mode and may mix standalone and bundle batches.
...
- Metadata requires exactly one enrolled signer, an unambiguous signer-to-program mapping, and at least one enrolled
  writable market update. Every writable instruction account other than the signer must be an enrolled market;
...
- These gates apply at `simple_forwarder`; the upstream Block Engine signature-verification and prefilter simulation
  stages continue independently. When the bundle gate is closed, their filtered output is consumed and discarded.
...
- Each round budgets through its projected end. Candidate fill applies a packing multiplier while speculative
  admission and commit remain gated by released block and account limits.
...
The auction has two configured execution engines:

- `legacy` (default): request/response speculative execution with drain and dispatch rounds.
- `wavefront`: graph workers pop ready nodes, commit directly into the speculative bank, and cascade newly ready
  dependents. Bundles execute alone inside a worker batch, while compatible standalone items may be packed together.

Both engines expose the same per-batch wire and result contract; packing and outer message grouping differ.
...
- Queue-send ordering depends on the Node scheduler engine. In default `legacy` mode, Node records the pending `seq_id`
  and forwarded telemetry, attempts the blocking queue send, and only then marks the DAG item sent when the callback
  returns, including after a closed queue or missing sender. In `wavefront` mode, the graph payload is already
  `SentToValidator` before the message callback. In both modes, a failed enqueue may already have forwarded telemetry
  and follows the sent-work `RetryableNextSlot` policy above rather than being immediately requeued.
