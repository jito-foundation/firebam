...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=os.getenv("FIREDANCER_LOG_PATH", os.getenv("FD_LOG_PATH")),
        help="path to the Firedancer log file used by --bam-only (default: newest autogenerated /tmp/fd-* log)",
    )
    p.add_argument(
        "--websocket-url",
        type=non_empty_string,
    ...
    if args.firedancer_log_path is not None:
        args.firedancer_log_path = str(Path(args.firedancer_log_path).expanduser())
...
def read_bam_endpoint_from_firedancer_log(raw_log_path: Optional[str]) -> tuple[Path, str, int]:
    ...
    zero_start_key = (0, 0, 0, 0, 0, 0, 0, 0)

    def parse_tmp_log_start(name: str) -> Optional[tuple[int, int, int, int, int, int, int, int]]:
        match = re.fullmatch(
            r"(?P<prefix>fd-.*)_(?P<year>\d{4})_(?P<month>\d{2})_(?P<day>\d{2})_"
            r"(?P<hour>\d{2})_(?P<minute>\d{2})_(?P<second>\d{2})_(?P<nanos>\d{9})_"
            r"(?P<tz>GMT[+_]\d{2})",
            name,
        )
        if match is None:
        ...
        tz_match = re.fullmatch(r"GMT(?P<sign>[+_])(?P<hours>\d{2})", match.group("tz"))
        if tz_match is None:
            return None

        tz_hours = int(tz_match.group("hours"))
        if tz_match.group("sign") == "_":
            tz_hours = -tz_hours

        try:
            nanos_int = int(match.group("nanos"))
            started_local = dt.datetime(
                ...
                microsecond=nanos_int // 1000,
                tzinfo=dt.timezone(dt.timedelta(hours=tz_hours)),
            )
        except ValueError:
            return None

        started_utc = started_local.astimezone(dt.timezone.utc)
        return (
            started_utc.year,
            started_utc.month,
            started_utc.day,
            ...
            started_utc.minute,
            started_utc.second,
            started_utc.microsecond,
            nanos_int % 1000,
        )

    if raw_log_path is None or raw_log_path.strip() in {"", "-"}:
        candidate = Path("/tmp")
        log_path: Optional[Path] = None
        search_dir = candidate
    else:
        candidate = Path(raw_log_path).expanduser()
        log_path = None if candidate.is_dir() else candidate
        search_dir = candidate if log_path is None else None

    if log_path is None:
        latest_entry: Optional[tuple[tuple[int, tuple[int, int, int, int, int, int, int, int], int, str], Path]] = None
        try:
            entries = list(search_dir.iterdir())
        except OSError as exc:
            raise RuntimeError(f"failed to list Firedancer logs under '{search_dir}': {exc}") from exc

        for entry in entries:
            if not entry.name.startswith("fd-") or not entry.is_file():
                continue
            ...
            start_key = parse_tmp_log_start(entry.name)
            candidate_key = (
                1 if start_key is not None else 0,
                start_key or zero_start_key,
                mtime_ns,
                entry.name,
            )
            if latest_entry is None or candidate_key > latest_entry[0]:
                latest_entry = (candidate_key, entry)

        if latest_entry is None:
            raise RuntimeError(f"no Firedancer logs matching '/tmp/fd-*' found under '{search_dir}'")
        log_path = latest_entry[1]
        ...
        with log_path.open("r", encoding="utf-8", errors="replace") as log_file:
            for line in log_file:
                for pattern in bam_log_endpoint_patterns:
                    ...
                    endpoint = (ip_text, port)
                    break
    except OSError as exc:
        raise RuntimeError(f"failed to read Firedancer log '{log_path}': {exc}") from exc

    if endpoint is None:
        raise RuntimeError(
            "could not find a BAM endpoint in Firedancer log "
            f"'{log_path}'; expected a BAM connect/connected line"
        )

    return log_path, endpoint[0], endpoint[1]
...
def scrape_websocket_slots(
    ...
    with WebsocketJsonSession(websocket_url) as ws:
        while time.monotonic() <= snapshot_deadline:
            ...
            topic = row.get("topic")
            key = row.get("key")
            if topic == "summary" and key in summary_keys and row.get("value") is not None:
    ...
    for row in snapshot:
        if row.get("topic") != "summary":
            continue
        key = row.get("key")
        value = row.get("value")
        if key in summary_keys and value is not None and key not in summary_values:
    ...
    try:
        ...
        raise RuntimeError("missing required summary fields in websocket snapshot") from exc
