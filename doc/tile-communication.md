...
The BAM tile has an external gRPC/TLS scheduler boundary. Inbound scheduler batches are converted into `bam_verif` fragments. Outbound feedback uses the same scheduler stream: `pack_bam_ldr` is sent as leader state messages, while `pack_bam_res`, `bank_bam`, and `poh_bam` feed result messages.
...
The BAM tile also has an external gRPC/TLS scheduler boundary. Inbound scheduler batches are converted into `bam_verif` fragments. Outbound feedback uses the same scheduler stream: `pack_bam_ldr` is sent as leader state messages, while `pack_bam_res`, `bank_bam`, and `poh_bam` feed result messages.
