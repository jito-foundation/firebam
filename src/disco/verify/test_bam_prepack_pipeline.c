...
mock_topo_create( void ) {
  ...
  fd_topo_tile_t * verify = fd_topob_tile( topo, "verify", "tilewksp", "tilewksp", 0UL, 0, 0, 0 );
  ...
  fd_topo_tile_t * dedup = fd_topob_tile( topo, "dedup", "tilewksp", "tilewksp", 0UL, 0, 0, 0 );
...
test_stem_init( test_stem_t *    stem,
  ...
  stem->stem = (fd_stem_context_t) {
    ...
    .out_reliable        = stem->out_reliable,
  };
}
...
run_duplicate_pipeline( ulong source_tpu,
  ...
  FD_TEST( verify_ctx->metrics.verify_tile_result[ FD_METRICS_ENUM_VERIFY_TILE_RESULT_V_DEDUP_FAILURE_IDX ] == expected_verify_dedup_fail_cnt );
  ...
  FD_TEST( dedup_ctx->metrics.dedup_tile_result[ FD_METRICS_ENUM_DEDUP_TILE_RESULT_V_DEDUP_FAILURE_IDX ] == expected_dedup_dedup_fail_cnt );
