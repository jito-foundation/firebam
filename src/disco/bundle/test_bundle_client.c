...
static void
test_bundle_bam_active_pause_resume( fd_wksp_t * wksp ) {
  test_bundle_env_t env[1];
  test_bundle_env_create( env, wksp );
  test_bundle_env_mock_conn( env );
  fd_bundle_tile_t * state = env->state;
  ...
  state->bundle_status_plugin = FD_BUNDLE_BLOCK_ENGINE_STATUS_CONNECTED;
  state->bundle_status_recent = FD_BUNDLE_BLOCK_ENGINE_STATUS_CONNECTED;
  state->bundle_status_logged = FD_BUNDLE_BLOCK_ENGINE_STATUS_CONNECTED;
  ...
  FD_TEST( state->bundle_status_recent == FD_BUNDLE_BLOCK_ENGINE_STATUS_DISCONNECTED );
  FD_TEST( state->bundle_status_logged == FD_BUNDLE_BLOCK_ENGINE_STATUS_DISCONNECTED );
...
main( int     argc,
  ...
  test_bundle_bam_active_pause_resume( wksp );
