...
fd_bundle_crank_gen_init( void                 * mem,
  ...
  uint  cerr[1];
  do {
    char seed[13] = "TIP_ACCOUNT_0"; /* Not NUL terminated */ /* fixed in main branch, don't keep change */
  ...
  do {
    char seed[14] = "CONFIG_ACCOUNT"; /* Not NUL terminated */ /* fixed in main branch, don't keep change */
    ulong seed_len = 14;
    uchar out_bump[1];
    uchar const * seed_ptr[1] = { (uchar const *)seed };
    FD_TEST( FD_PUBKEY_SUCCESS==fd_pubkey_find_program_address( (fd_pubkey_t const *)tip_payment_program_addr,
