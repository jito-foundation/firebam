...
#define SHREDS_REV_ITER( age_ns, code_staged, code_archive ) \
        do { \
          ...
          for( ulong i=gui->shreds.staged_tail; i>gui->shreds.staged_head; i-- ) { \
            fd_gui_slot_staged_shred_event_t * event = &gui->shreds.staged[ (i-1UL) % FD_GUI_SHREDS_STAGING_SZ ]; \
...
fd_gui_printf_bam( fd_gui_t * gui ) {
  jsonp_open_envelope( gui->http, "bam", "update" );
    jsonp_open_object( gui->http, "value" );
      ...
      jsonp_ulong(  gui->http, "outbound_heartbeat_enqueued", gui->bam.outbound_heartbeat_enqueued );
      jsonp_ulong(  gui->http, "outbound_heartbeat_enqueue_fail", gui->bam.outbound_heartbeat_enqueue_fail );
      ...
      jsonp_ulong(  gui->http, "failure_resolve", gui->bam.failure_resolve );
      jsonp_ulong(  gui->http, "failure_connect", gui->bam.failure_connect );
      jsonp_ulong(  gui->http, "failure_io", gui->bam.failure_io );
      jsonp_ulong(  gui->http, "failure_unsupported_version", gui->bam.failure_unsupported_version );
      jsonp_ulong(  gui->http, "failure_request_timeout", gui->bam.failure_request_timeout );
      jsonp_ulong(  gui->http, "failure_keepalive_timeout", gui->bam.failure_keepalive_timeout );
      jsonp_ulong(  gui->http, "failure_builder_activity_timeout", gui->bam.failure_builder_activity_timeout );
    ...
    jsonp_open_object( peers->http, "value" );
      jsonp_open_array( peers->http, "changes" );
        ...
        for( fd_gui_peers_live_table_fwd_iter_t iter = fd_gui_peers_live_table_fwd_iter_init( peers->live_table, &peers->client_viewports[ ws_conn_id ].sort_key, peers->contact_info_table ), j = 0;
          ...
          long cur_egress_push_kbps           = cur->row.gossip_tx[ FD_METRICS_ENUM_GOSSIP_MESSAGE_V_PUSH_IDX ].rate_ema;
          long ref_egress_push_kbps           = ref->gossip_tx    [ FD_METRICS_ENUM_GOSSIP_MESSAGE_V_PUSH_IDX ].rate_ema;
          long cur_ingress_push_kbps          = cur->row.gossvf_rx[ FD_METRICS_ENUM_GOSSIP_MESSAGE_V_PUSH_IDX ].rate_ema;
          long ref_ingress_push_kbps          = ref->gossvf_rx    [ FD_METRICS_ENUM_GOSSIP_MESSAGE_V_PUSH_IDX ].rate_ema;
          long cur_egress_pull_response_kbps  = cur->row.gossip_tx[ FD_METRICS_ENUM_GOSSIP_MESSAGE_V_PULL_RESPONSE_IDX ].rate_ema;
          long ref_egress_pull_response_kbps  = ref->gossip_tx    [ FD_METRICS_ENUM_GOSSIP_MESSAGE_V_PULL_RESPONSE_IDX ].rate_ema;
          long cur_ingress_pull_response_kbps = cur->row.gossvf_rx[ FD_METRICS_ENUM_GOSSIP_MESSAGE_V_PULL_RESPONSE_IDX ].rate_ema;
