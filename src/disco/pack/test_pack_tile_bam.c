...
fd_txn_e_t * fd_pack_insert_txn_init( fd_pack_t * );
int fd_pack_insert_txn_fini( fd_pack_t *, fd_txn_e_t *, ulong, ulong * );
ulong fd_pack_delete_transaction( fd_pack_t *, fd_ed25519_sig_t const * );
void fd_pack_insert_bundle_cancel( fd_pack_t *, fd_txn_e_t * const *, ulong );
...
static int               test_cancel_use_real_pack;
...
test_fd_pack_insert_bundle_cancel( fd_pack_t *          pack,
  ...
  if( test_cancel_use_real_pack ) fd_pack_insert_bundle_cancel( pack, bundle, txn_cnt );
}
...
test_pack_tile_out_new( test_pack_tile_out_t * out ) {
  ...
  out->mcache = aligned_alloc( alignof(fd_frag_meta_t), 4UL * TEST_PACK_TILE_MCACHE_DEPTH * sizeof(fd_frag_meta_t) );
  FD_TEST( out->mcache );
  fd_memset( out->mcache, 0, 4UL * TEST_PACK_TILE_MCACHE_DEPTH * sizeof(fd_frag_meta_t) );
  ...
  for( ulong i=0UL; i<4UL; i++ ) {
    out->mcaches[i] = out->mcache+i*TEST_PACK_TILE_MCACHE_DEPTH;
    out->depths[i] = TEST_PACK_TILE_MCACHE_DEPTH;
    out->cr_avail[i] = ULONG_MAX;
    out->out_reliable[i] = 1;
  }
...
pack_tile_bam_work_find_by_sig0( fd_pack_ctx_t const * ctx,
  ...
  for( ulong i=0UL; i<ctx->bam_work_cnt; i++ ) {
    if( !(ctx->bam_work[i].indexed_mask & 1U) ) continue;
...
pack_tile_track_bam_work( fd_pack_ctx_t * ctx,
  ...
  return pack_tile_append_bam_work( ctx, sigs, first_rx_ts_ns, seq_id, scheduler_gen, max_schedule_slot, blockhash_slot, txn_cnt );
}
...
test_pack_tile_mark_bam_work_scheduled( test_pack_tile_harness_t * h,
  ...
  FD_TEST( work_idx>=h->ctx->bam_scheduled_work_cnt );
...
test_pack_tile_bam_work_partition_mutations( void ) {
  ...
  for( uchar i=0U; i<3U; i++ ) {
    ...
    FD_TEST( pack_tile_track_bam_work( h->ctx, sigs[ i ], 0L, (uint)i, 0U, 100UL, 100UL, 1U ) );
  }
  ...
  FD_TEST( pack_tile_bam_pending_work_cnt( h->ctx )==1UL );
  FD_TEST( h->ctx->bam_work[ 0 ].seq_id==1U );
  FD_TEST( h->ctx->bam_work[ 1 ].seq_id==2U );
  FD_TEST( h->ctx->bam_work[ 2 ].seq_id==0U );
  ...
  FD_TEST( h->ctx->bam_work[ 1 ].seq_id==0U );
...
test_pack_tile_bam_signature_prefix_collision( void ) {
  ...
  FD_TEST( pack_tile_track_bam_work( h->ctx, sigs[ 0 ], 0L, 100U, 0U, 100UL, 100UL, 1U ) );
  FD_TEST( pack_tile_track_bam_work( h->ctx, sigs[ 1 ], 0L, 101U, 0U, 100UL, 100UL, 1U ) );

  ulong work_idx = pack_tile_bam_work_find( h->ctx, sigs[ 1 ], 0, NULL );
  FD_TEST( work_idx<h->ctx->bam_work_cnt );
  FD_TEST( h->ctx->bam_work[ work_idx ].seq_id==101U );

  uchar matched_idx = UCHAR_MAX;
  work_idx = pack_tile_bam_work_find( h->ctx, sigs[ 1 ], 0, &matched_idx );
  FD_TEST( work_idx<h->ctx->bam_work_cnt );
  FD_TEST( h->ctx->bam_work[ work_idx ].seq_id==101U );
...
test_pack_tile_bam_completion_outcomes( void ) {
  ...
  for( ulong case_idx=0UL; case_idx<sizeof(cases)/sizeof(*cases); case_idx++ ) {
    ...
    FD_TEST( pack_tile_track_bam_work( h->ctx, sigs, 0L, (uint)( case_idx+1UL ), 0U, 100UL, 100UL, txn_cnt ) );
...
test_pack_tile_bam_completion_tracking_reuses_capacity( void ) {
  ...
  FD_TEST( pack_tile_track_bam_work( h->ctx, sigs, 0L, 5U, 0U, 100UL, 100UL, 2U ) );
  (void)test_pack_tile_mark_bam_work_scheduled( h, sigs[ 0 ] );
  ...
  FD_TEST( h->ctx->bam_work[ 0 ].indexed_mask == 2U );
  ...
  for( ulong i=0UL; i<3UL*TEST_PACK_TILE_BAM_WORK_CAP; i++ ) {
    ...
    FD_TEST( pack_tile_track_bam_work( h->ctx, sigs[ 0 ], 0L, (uint)( 100UL + i ), 0U, 100UL, 100UL, 1U ) );
    (void)test_pack_tile_mark_bam_work_scheduled( h, sigs[ 0 ] );
    test_pack_tile_send_executed_txn( h, sigs[ 0 ], FD_EXECUTED_TXN_KIND_BAM_COMPLETED_UNLANDED );
    FD_TEST( h->ctx->bam_work_cnt == 0UL );
  }
...
test_pack_tile_bam_overlapping_replay_reconciliation( void ) {
  ...
  for( uchar i=0U; i<3U; i++ ) {
    ...
    FD_TEST( pack_tile_track_bam_work( h->ctx, sigs, 0L, 110U+i, 8U, 101UL, 101UL, 2U ) );
  }
...
test_pack_tile_bam_ownership_generation_retirement_barrier( void ) {
  ...
  FD_TEST( pack_tile_track_bam_work( h->ctx, pending_old_sig,   0L, 10U, 7U, ULONG_MAX, 100UL, 1U ) );
  FD_TEST( pack_tile_track_bam_work( h->ctx, scheduled_old_sig, 0L, 11U, 7U, ULONG_MAX, 100UL, 1U ) );
  ...
  test_pack_tile_send_bam_resolv_frag( h, 13U, 7U, 7U, ULONG_MAX, 0U, 1U, 0, 100UL, 40U );
  FD_TEST( h->ctx->bam_work_cnt==1UL );
...
/* Retire old BAM work before clearing override.  Readiness also prevents
   buffered BAM work from becoming eligible for ordinary scheduling. */
static void
test_pack_tile_bam_disable_retires_pending_before_override_clear( void ) {
  ...
  /* Without retirement the work remains buffered but cannot dispatch.
     The generation barrier removes that work before acknowledging handoff. */
  for( int retire_before_clear=0; retire_before_clear<2; retire_before_clear++ ) {
    ...
    /* Even a buffered candidate cannot inherit normal-mode readiness.
       The generation barrier still owns retirement of pending BAM work. */
    FD_TEST( scheduled_cnt==0UL );
    FD_TEST( fd_pack_avail_txn_cnt( pack )==(ulong)!retire_before_clear );
...
test_pack_tile_complete_bam_bundle( test_pack_tile_harness_t * h,
  ...
  (void)min_blockhash_slot_txn_idx;
...
test_pack_tile_bam_v1_signature_lifecycle( void ) {
  test_pack_tile_harness_t h[1];
  fd_txn_e_t               first[1]   = {0};
  fd_txn_e_t               resend[1]  = {0};
  fd_txn_e_t               cleanup[1] = {0};
  ...
  pack_tile_bam_work_mark_txn_scheduled( h->ctx, resend->txnp, 1200L );
  FD_TEST( pack_tile_bam_work_find( h->ctx, resend_sig, 1, NULL )<h->ctx->bam_work_cnt );
...
/* Build an AdvanceNonce transaction with either an instruction signer or an
   unsigned authority, while keeping a valid message signature. */
static void
test_pack_tile_make_d26_poc_txn( fd_txn_p_t * txnp,
  ...
  *p++ = (uchar)fd_int_if( signed_auth, 0, 3 ); /* fee payer signer or unsigned authority */
  p += fd_cu16_enc( 4U, p );
  ...
  FD_TEST( txnp->payload[ txn->instr[ 0 ].acct_off+2UL ]==(uchar)fd_int_if( signed_auth, 0, 3 ) );
  FD_TEST( fd_txn_is_signer( txn, txnp->payload[ txn->instr[ 0 ].acct_off+2UL ] )==!!signed_auth );
}
  ...
  for( uchar i=0U; i<3U; i++ ) {
    ...
    for( uchar txn_idx=0U; txn_idx<2U; txn_idx++ ) {
      *bundle[ txn_idx ]->txnp = *valid_txn;
      bundle[ txn_idx ]->first_seen_nanos = 0L;
...
test_pack_tile_bam_stale_max_schedule_slot_rejected( void ) {
  ...
  FD_TEST( pack_tile_track_bam_work( h->ctx, sigs, 0L, 55U, 0U, 100UL, 100UL, 2U ) );
  FD_TEST( h->ctx->bam_work_cnt == 1UL );
  FD_TEST( pack_tile_bam_pending_work_cnt( h->ctx ) == 1UL );

  h->ctx->bam_min_admission_slot = 101UL;
  pack_tile_evict_invalid_pending_bam_work( h->ctx );
...
test_pack_tile_bam_pack_evicted_bundle_is_reconciled( void ) {
  ...
  FD_TEST( pack_tile_bam_pending_work_cnt( h->ctx )==pack_depth );
  ...
  *overflow[ 0 ]->txnp = *valid_txn;
  overflow[ 0 ]->first_seen_nanos = 0L;
...
test_pack_tile_bam_sig0_shared_with_untracked_pack_txn_is_accepted( void ) {
  ...
  *untracked[ 0 ]->txnp = *valid_txn;
  untracked[ 0 ]->first_seen_nanos = 0L;
...
test_pack_tile_bam_overlapping_bundle_leading_sig_is_accepted( void ) {
  ...
  *bundle_a[ 0 ]->txnp = *valid_txn;
  bundle_a[ 0 ]->first_seen_nanos = 0L;
  *bundle_a[ 1 ]->txnp = *valid_txn;
  bundle_a[ 1 ]->first_seen_nanos = 0L;
  ...
  *bundle_b[ 0 ]->txnp = *valid_txn;
  bundle_b[ 0 ]->first_seen_nanos = 0L;
  *bundle_b[ 1 ]->txnp = *valid_txn;
  bundle_b[ 1 ]->first_seen_nanos = 0L;
  ...
  FD_TEST( test_bundle_cancel_call_cnt==cancel_before );
  FD_TEST( pack_tile_bam_pending_work_cnt( h->ctx )==2UL );
  FD_TEST( pack_tile_bam_work_find_by_sig0( h->ctx, sig_y )<h->ctx->bam_work_cnt );
  ...
  *bundle_b_resend[ 0 ]->txnp = *valid_txn;
  bundle_b_resend[ 0 ]->first_seen_nanos = 0L;
  *bundle_b_resend[ 1 ]->txnp = *valid_txn;
  bundle_b_resend[ 1 ]->first_seen_nanos = 0L;
...
/* An old member reference slot cannot override the batch's missed target.
   Report one OUTSIDE_SLOT result only after all members have arrived. */
static void
test_pack_tile_bam_old_member_does_not_override_missed_target( void ) {
  for( ulong revert_on_error=0UL; revert_on_error<2UL; revert_on_error++ ) {
    ...
    h->ctx->bam_min_admission_slot = max_schedule_slot+1UL;
    ...
    for( uchar batch_idx=1U; batch_idx<3U; batch_idx++ ) {
      test_pack_tile_send_bam_resolv_frag( h,
                                           ...
                                           3U,
                                           (_Bool)revert_on_error,
                                           batch_idx==1U ? 10UL : blockhash_slot,
    ...
    FD_TEST( pack_tile_drain_one_pending_bam_result( h->ctx, &h->out->stem ) );
    fd_bam_bundle_result_t const * rejected = test_pack_tile_assert_last_result( h,
                                                                                seq_id,
                                                                                max_schedule_slot,
                                                                                3U,
                                                                                FD_BAM_SCHED_ERR_OUTSIDE_SLOT,
                                                                                0U );
    FD_TEST( !rejected->transaction_err_count );
    FD_TEST( h->ctx->bam_pending_result_cnt==0UL );
...
test_pack_tile_bam_stale_results_drain_without_drop( void ) {
  ...
  for( uchar i=0U; i<3U; i++ ) {
    ...
    FD_TEST( pack_tile_track_bam_work( h->ctx, sigs[ i ], 0L, (uint)( 100U + i ), 0U, 100UL, 100UL, 1U ) );
  }
...
test_pack_tile_bam_same_seq_pending_duplicate_replaces_before_insert( void ) {
  ...
  new_txn->first_seen_nanos = 20L;

  FD_TEST( pack_tile_track_bam_work( h->ctx, old_sigs, 10L, 10U, 0U, 100UL, 100UL, 2U ) );
  FD_TEST( h->ctx->bam_work_cnt == 1UL );
  FD_TEST( pack_tile_bam_pending_work_cnt( h->ctx ) == 1UL );
  FD_TEST( h->ctx->bam_work_item_stage_cnt[ FD_METRICS_ENUM_PACK_BAM_WORK_STAGE_V_PENDING_ENTERED_IDX ] == 1UL );
...
test_pack_tile_bam_scheduled_duplicate_rejected_before_insert( void ) {
  ...
  FD_TEST( pack_tile_track_bam_work( h->ctx, sig, 10L, 20U, 0U, 100UL, 100UL, 1U ) );
  ...
  new_txns[ 0 ].first_seen_nanos = 20L;
  new_txns[ 1 ].first_seen_nanos = 21L;
...
test_pack_tile_bam_pending_result_does_not_shadow_new_work( void ) {
  ...
  FD_TEST( pack_tile_track_bam_work( h->ctx, sig, 0L, 200U, 0U, 100UL, 100UL, 1U ) );
  h->ctx->bam_min_admission_slot = 101UL;
  pack_tile_evict_invalid_pending_bam_work( h->ctx );
  ...
  FD_TEST( pack_tile_track_bam_work( h->ctx, sig, 0L, 201U, 0U, 102UL, 102UL, 1U ) );
...
test_pack_tile_bam_queued_results_preserve_fifo_before_direct_publish( void ) {
  ...
  FD_TEST( pack_tile_track_bam_work( h->ctx, sig, 0L, 100U, 0U, 100UL, 100UL, 1U ) );
  h->ctx->bam_min_admission_slot = 101UL;
  pack_tile_evict_invalid_pending_bam_work( h->ctx );
...
test_pack_tile_bam_pending_results_reserve_direct_result_headroom( void ) {
  ...
  for( uchar i=0U; i<TEST_PACK_TILE_BAM_WORK_CAP; i++ ) {
    ...
    FD_TEST( pack_tile_track_bam_work( h->ctx, sigs[ i ], 0L, (uint)( 300U + i ), 0U, 100UL, 100UL, 1U ) );
  }
  ...
  for( uchar i=0U; i<TEST_PACK_TILE_BAM_WORK_CAP-1U; i++ ) {
    ...
    FD_TEST( pack_tile_track_bam_work( h->ctx, sigs[ i ], 0L, (uint)( 400U + i ), 0U, 102UL, 102UL, 1U ) );
  }
  test_pack_tile_fill_sig( sigs[ TEST_PACK_TILE_BAM_WORK_CAP-1U ], 130U );
  FD_TEST( !pack_tile_track_bam_work( h->ctx, sigs[ TEST_PACK_TILE_BAM_WORK_CAP-1U ], 0L, 499U, 0U, 102UL, 102UL, 1U ) );
...
test_pack_tile_bam_scheduled_work_does_not_consume_result_headroom( void ) {
  ...
  for( uchar i=0U; i<4U; i++ ) {
    ...
    FD_TEST( pack_tile_track_bam_work( h->ctx, sigs[ i ], 0L, (uint)( 500U + i ), 0U, 200UL, 200UL, 1U ) );
  ...
  FD_TEST( pack_tile_track_bam_work( h->ctx, sigs[ 4 ], 0L, 600U, 0U, 201UL, 201UL, 1U ) );
...
test_pack_tile_bam_instr_acct_reject_serializes_exact_member( void ) {
  ...
  FD_TEST( fd_bam_flush_results( env->state )==1 );
...
/* An unsigned AdvanceNonce instruction can carry a live recent hash.  Pack
   must leave both singleton and later bundle-member forms pending so the
   execution bank can decide whether to charge and commit an instruction failure. */
static void
test_pack_tile_bam_unsigned_advance_passes_pack( void ) {
  for( uchar unsigned_idx=0U; unsigned_idx<2U; unsigned_idx++ ) {
    uchar txn_cnt = (uchar)(unsigned_idx+1U);
    uint  seq_id  = unsigned_idx ? 12U : 26U;
    ...
    fd_rng_t pack_rng[1];
    FD_TEST( fd_rng_join( fd_rng_new( pack_rng, 0U, (ulong)unsigned_idx ) ) );
    ...
    test_pack_tile_make_d26_poc_txn( bundle[ unsigned_idx ]->txnp, 0 );
    for( ulong i=0UL; i<txn_cnt; i++ ) bundle[ i ]->first_seen_nanos = 0L;
    ...
    FD_TEST( h->ctx->bam_work_cnt==1UL && pack_tile_bam_pending_work_cnt( h->ctx )==1UL );
    FD_TEST( !h->ctx->bam_pending_result_cnt );
    test_pack_tile_harness_delete( h );
...
test_pack_tile_bam_atomic_abandon_result_mapping( void ) {
  ...
  for( ulong reason_idx=0UL; reason_idx<sizeof(reasons)/sizeof(reasons[0]); reason_idx++ ) {
    ...
    old_txns[ 0 ].first_seen_nanos = 0L;
    old_txns[ 1 ].first_seen_nanos = 0L;
...
/* Drive the actual Pack callbacks with a real two-worker pack and distinct
   executor links.  The other tile tests retain their small fake Pack. */
typedef struct {
  test_pack_tile_harness_t h[1];
  fd_rng_t rng[1];
  void * pack_mem;
  void * exec_dcache_mem[2];
  void * extra_mem;
  ulong status;
  ulong generation;
  ulong busy[2];
} test_pack_callbacks_t;

static void
test_pack_callbacks_new( test_pack_callbacks_t * e,
                         int                     strategy ) {
  fd_memset( e, 0, sizeof(*e) );
  test_pack_tile_harness_new( e->h );
  ...
  FD_TEST( fd_rng_join( fd_rng_new( e->rng, 0U, 0UL ) ) );
  ulong sz = fd_pack_footprint( TEST_PACK_TILE_BAM_WORK_CAP, BUNDLE_META_SZ, 2UL, &limits );
  e->pack_mem = aligned_alloc( fd_pack_align(), fd_ulong_align_up( sz, fd_pack_align() ) );
  FD_TEST( e->pack_mem );
  ctx->pack = fd_pack_join( fd_pack_new( e->pack_mem, TEST_PACK_TILE_BAM_WORK_CAP, BUNDLE_META_SZ,
                                       2UL, &limits, NULL, 0UL, e->rng ) );
  FD_TEST( ctx->pack );
  test_insert_fini_use_real_pack = test_delete_use_real_pack = test_cancel_use_real_pack = 1;
  ctx->execle_cnt = 2UL;
  ctx->execle_idle_bitset = 3UL;
  ctx->strategy = strategy;
  e->status = FD_BAM_STATUS_FSEQ_OVERRIDE_ACTIVE;
  e->generation = 7UL<<1;
  ctx->bam_status_fseq = &e->status;
  ctx->bam_gen_fseq = &e->generation;
  ctx->bam_ownership_gen = 7U;
  ctx->bam_override_snapshot = 1;
  ctx->bam_leader_out.out_idx = ULONG_MAX;
  ctx->bam_result_out.out_idx = 2UL;
  ctx->poh_out = (fd_pack_out_ctx_t){ .out_idx=1UL, .mem=(fd_wksp_t *)e->h->out->dcache,
      .chunk0=ctx->bam_result_out.chunk0, .chunk=ctx->bam_result_out.chunk0, .wmark=ctx->bam_result_out.wmark };
  for( ulong i=0UL; i<2UL; i++ ) {
    ulong data_sz = fd_dcache_req_data_sz( MAX_MICROBLOCK_SZ, TEST_PACK_TILE_MCACHE_DEPTH, 1UL, 1 );
    e->exec_dcache_mem[i] = aligned_alloc( fd_dcache_align(), fd_dcache_footprint( data_sz, 0UL ) );
    FD_TEST( e->exec_dcache_mem[i] );
    void * dc = fd_dcache_join( fd_dcache_new( e->exec_dcache_mem[i], data_sz, 0UL ) );
    FD_TEST( dc );
    ulong chunk0 = fd_dcache_compact_chunk0( dc, dc );
    ctx->execle_out[i] = (fd_pack_out_ctx_t){ .out_idx=i ? 3UL : 0UL, .mem=dc,
        .chunk0=chunk0, .chunk=chunk0, .wmark=fd_dcache_compact_wmark( dc, dc, MAX_MICROBLOCK_SZ ) };
    e->busy[i] = ULONG_MAX;
    ctx->execle_current[i] = &e->busy[i];
    ctx->execle_expect[i] = 0UL;
  }
#if FD_PACK_USE_EXTRA_STORAGE
  e->extra_mem = aligned_alloc( extra_txn_deq_align(), fd_ulong_align_up( extra_txn_deq_footprint(), extra_txn_deq_align() ) );
  FD_TEST( e->extra_mem );
  ctx->extra_txn_deq = extra_txn_deq_join( extra_txn_deq_new( e->extra_mem ) );
#endif
  fd_histf_new( ctx->schedule_duration, 1UL, 1000000UL );
  fd_histf_new( ctx->no_sched_duration, 1UL, 1000000UL );
  fd_histf_new( ctx->insert_duration, 1UL, 1000000UL );
  fd_histf_new( ctx->complete_duration, 1UL, 1000000UL );
  fd_pack_pacing_init( ctx->pacer, fd_tickcount(), fd_tickcount()+1000000000L, 1.0f, limits.max_cost_per_block );
}

static void
test_pack_callbacks_delete( test_pack_callbacks_t * e ) {
  fd_pack_delete( fd_pack_leave( e->h->ctx->pack ) );
  fd_rng_delete( fd_rng_leave( e->rng ) );
  free( e->pack_mem );
  for( ulong i=0UL; i<2UL; i++ ) free( e->exec_dcache_mem[i] );
  free( e->extra_mem );
  test_pack_tile_harness_delete( e->h );
}

static void
test_pack_callbacks_leader( test_pack_callbacks_t * e,
                            ulong                   slot,
                            int                     frank ) {
  fd_pack_ctx_t * ctx = e->h->ctx;
  fd_became_leader_t leader = {0};
  leader.slot = leader.bank_idx = slot;
  leader.bank_seq = slot+1000UL;
  leader.slot_start_ns = fd_log_wallclock();
  leader.slot_end_ns = leader.slot_start_ns+400000000L;
  leader.tick_duration_ns = 6250000UL;
  leader.ticks_per_slot = 64UL;
  leader.max_microblocks_in_slot = 1024UL;
  leader.limits.slot_max_cost = FD_PACK_MAX_COST_PER_BLOCK_LOWER_BOUND;
  leader.limits.slot_max_vote_cost = FD_PACK_MAX_VOTE_COST_PER_BLOCK_LOWER_BOUND;
...
static void
test_pack_callbacks_insert_with_payer( test_pack_callbacks_t * e,
                            uint                    seq_id,
                            ulong                   target_slot,
                            int                     revert,
                            ulong                   reference_slot,
                            uchar                   payer_seed ) {
  fd_txn_p_t invalid[1], txn[1];
  test_pack_tile_make_d18_poc_txns( invalid, txn );
  test_pack_tile_fill_sig( txn->payload+1UL, (uchar)seq_id );
  txn->payload[ TXN(txn)->acct_addr_off ] ^= payer_seed;
  uchar resolved[ FD_TPU_RESOLVED_MTU ] __attribute__((aligned(FD_CHUNK_ALIGN)));
  ulong leader_slot = e->h->ctx->leader_slot;
  ulong sz = test_pack_tile_prepare_resolv_frag( e->h, resolved, txn->payload, txn->payload_sz, FD_TXN_M_TPU_SOURCE_BAM, reference_slot );
  e->h->ctx->leader_slot = leader_slot;
  fd_txn_m_t * m = (fd_txn_m_t *)resolved;
  m->bam.max_schedule_slot = target_slot;
  m->bam.seq_id = seq_id;
  m->bam.scheduler_gen = 9U;
  m->bam.ownership_gen = e->h->ctx->bam_ownership_gen;
  m->bam.txn_cnt = 1U;
  m->bam.revert_on_error = !!revert;
  m->block_engine.bundle_id = revert ? (ulong)seq_id+1UL : 0UL;
  m->block_engine.bundle_txn_cnt = revert ? 1UL : 0UL;
  during_frag( e->h->ctx, 0UL, 0UL, reference_slot, 0UL, sz, 0UL );
  after_frag( e->h->ctx, 0UL, 0UL, reference_slot, sz, 0UL, 0UL, &e->h->out->stem );
}

static void
test_pack_callbacks_insert_at( test_pack_callbacks_t * e,
                               uint                    seq_id,
                               ulong                   target_slot,
                               int                     revert,
                               ulong                   reference_slot ) {
  test_pack_callbacks_insert_with_payer( e, seq_id, target_slot, revert, reference_slot, (uchar)seq_id );
}

static void
test_pack_callbacks_insert( test_pack_callbacks_t * e,
                            uint                    seq_id,
                            ulong                   target_slot,
                            int                     revert ) {
  test_pack_callbacks_insert_at( e, seq_id, target_slot, revert, 100UL );
}

static void
test_pack_callbacks_step( test_pack_callbacks_t * e ) {
  e->h->ctx->skip_cnt = 0L;
  int poll=1, busy=0;
  after_credit( e->h->ctx, &e->h->out->stem, &poll, &busy );
}

static ulong
test_pack_callbacks_dispatch_count( test_pack_callbacks_t const * e ) {
  return e->h->out->seqs[0]+e->h->out->seqs[3];
}

static fd_txn_e_t const *
test_pack_callbacks_output( test_pack_callbacks_t const * e,
                            ulong                         ordinal,
                            fd_microblock_execle_trailer_t const ** trailer ) {
  /* These fixtures dispatch one transaction per fragment.  Locate the
     global Pack index across the two executor rings. */
  for( ulong worker=0UL; worker<2UL; worker++ ) {
    ulong idx = worker ? 3UL : 0UL;
    for( ulong seq=0UL; seq<e->h->out->seqs[idx]; seq++ ) {
      fd_frag_meta_t const * m = &e->h->out->mcaches[idx][fd_mcache_line_idx( seq, TEST_PACK_TILE_MCACHE_DEPTH )];
      fd_txn_e_t const * txns = fd_chunk_to_laddr_const( e->h->ctx->execle_out[worker].mem, m->chunk );
      ulong count = (m->sz-sizeof(**trailer))/sizeof(fd_txn_e_t);
      *trailer = (fd_microblock_execle_trailer_t const *)(txns+count);
      if( (*trailer)->microblock_idx==ordinal ) return txns;
    }
  }
  FD_LOG_ERR(( "missing Pack output %lu", ordinal ));
}

static void
test_pack_tile_bam_target_slot_callbacks( void ) {
  for( int strategy=FD_PACK_STRATEGY_PERF; strategy<=FD_PACK_STRATEGY_BALANCED; strategy++ )
  for( int frank=0; frank<2; frank++ )
  for( int revert=0; revert<2; revert++ ) {
    test_pack_callbacks_t e[1];
    test_pack_callbacks_new( e, strategy );
    fd_pack_ctx_t * ctx = e->h->ctx;
    test_pack_callbacks_leader( e, 104UL, frank );
    test_pack_callbacks_insert( e, 41U, 105UL, revert );
    FD_TEST( !ctx->bam_current_slot_has_bam_work );
    test_pack_callbacks_insert( e, 42U, 104UL, revert );
    FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==2UL );
    ...
    FD_TEST( ctx->pack_idx==0U && !ctx->slot_microblock_cnt );
    ...
    FD_TEST( ctx->bam_min_admission_slot==105UL && ctx->drain_execle );
    FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==1UL );
    FD_TEST( ctx->bam_pending_result_cnt==1UL );
    uint after_close_idx = ctx->pack_idx;
    test_pack_callbacks_step( e );
    FD_TEST( ctx->pack_idx==after_close_idx && !test_pack_callbacks_dispatch_count( e ) );
    /* Result progress is independent of old executor completion. */
    FD_TEST( !ctx->bam_pending_result_cnt && e->h->out->seqs[2]==1UL );
    fd_frag_meta_t const * res_meta = e->h->out->mcaches[2];
    fd_bam_bundle_result_t const * res = fd_chunk_to_laddr_const( e->h->out->dcache, res_meta->chunk );
    FD_TEST( res->seq_id==42U && res->scheduling_error==FD_BAM_SCHED_ERR_OUTSIDE_SLOT );
    e->busy[0] = ctx->execle_expect[0];
    test_pack_callbacks_step( e ); /* done draining */
    FD_TEST( !ctx->drain_execle && !test_pack_callbacks_dispatch_count( e ) );
    test_pack_callbacks_step( e );
    FD_TEST( test_pack_callbacks_dispatch_count( e )==1UL && ctx->bam_scheduled_work_cnt==1UL );
    fd_microblock_execle_trailer_t const * trailer;
    fd_txn_e_t const * txn = test_pack_callbacks_output( e, 0UL, &trailer );
    FD_TEST( txn->txnp->bam.seq_id==41U && txn->txnp->bam.revert_on_error==(uchar)revert );
    FD_TEST( trailer->bank_idx==105UL && trailer->bank_seq==1105UL );
    FD_TEST( trailer->pack_idx==after_close_idx );

    test_pack_callbacks_insert( e, 45U, ULONG_MAX, revert );
    FD_TEST( !pack_tile_bam_pending_work_cnt( ctx ) && ctx->bam_pending_result_cnt==1UL );
    res = &ctx->bam_result_queue[ ctx->bam_result_queue_head ];
    FD_TEST( res->seq_id==45U && res->scheduling_error==FD_BAM_SCHED_ERR_OUTSIDE_SLOT );
    test_pack_callbacks_delete( e );
  }
}

static void
test_pack_tile_bam_secondary_callbacks( void ) {
  for( int revert=0; revert<2; revert++ ) {
    test_pack_callbacks_t e[1];
    ...
    for( ulong i=0UL; i<2UL; i++ ) {
      ...
      FD_TEST( txn->txnp->bam.seq_id==(uint)(51UL+i) && trailer->pack_idx==(uint)i );
      FD_TEST( trailer->is_bundle==(uchar)revert && trailer->bank_idx==104UL );
    }
    e->busy[1] = ctx->execle_expect[1];
    for( int i=0; i<2; i++ ) test_pack_callbacks_step( e );
    FD_TEST( ctx->execle_idle_bitset==2UL && ctx->pack_idx==2U );
    test_pack_callbacks_delete( e );
  }
}

static void
test_pack_tile_bam_conflict_lookahead_callbacks( void ) {
  for( int frank=0; frank<2; frank++ )
  for( int revert=0; revert<2; revert++ )
  for( int scenario=0; scenario<7; scenario++ ) {
    ...
    fd_pack_ctx_t * ctx = e->h->ctx;
    test_pack_callbacks_leader( e, 104UL, frank );
    test_pack_callbacks_insert_with_payer( e, 81U, 104UL, revert, 100UL, 11U );
    test_pack_callbacks_step( e );
    FD_TEST( test_pack_callbacks_dispatch_count( e )==1UL && ctx->execle_idle_bitset==2UL );
    test_pack_callbacks_insert_with_payer( e, 82U, scenario==1 ? 105UL : 104UL, revert, 100UL, 11U );
    test_pack_callbacks_insert_with_payer( e, 83U, scenario==2 ? 105UL : 104UL, revert, 100UL, scenario==6 ? 11U : 12U );
    if( scenario==1 || scenario==2 )
      FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==2UL && !ctx->bam_pending_result_cnt );
    /* Readiness for each skipped/selected batch comes from its own sidecar. */
    for( ulong i=ctx->bam_scheduled_work_cnt; i<ctx->bam_work_cnt; i++ ) {
      pack_bam_work_t * w = &ctx->bam_work[i];
      if( w->seq_id==83U && scenario==3 ) w->scheduler_gen++;
      if( w->seq_id==83U && scenario==4 ) w->txn_cnt++;
    }
    if( scenario==5 ) ctx->bam_override_snapshot = 0;
    if( scenario==5 ) e->status = 0UL;
    for( ulong i=0UL; i<4UL; i++ ) test_pack_callbacks_step( e );
    ulong expected = scenario==0 ? 2UL : 1UL;
    FD_TEST( test_pack_callbacks_dispatch_count( e )==expected );
    FD_TEST( ctx->bam_scheduled_work_cnt==expected );
    if( scenario==0 ) {
      fd_microblock_execle_trailer_t const * trailer;
      fd_txn_e_t const * txn = test_pack_callbacks_output( e, 1UL, &trailer );
      FD_TEST( txn->txnp->bam.seq_id==83U && trailer->bank_idx==104UL );
      FD_TEST( trailer->is_bundle==(uchar)revert && ctx->pack_idx==2U );
      FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==1UL );
      /* B keeps its original ordering behind A despite C's dispatch. */
      e->busy[0] = ctx->execle_expect[0];
      for( ulong i=0UL; i<4UL; i++ ) test_pack_callbacks_step( e );
      FD_TEST( test_pack_callbacks_dispatch_count( e )==3UL );
      txn = test_pack_callbacks_output( e, 2UL, &trailer );
      FD_TEST( txn->txnp->bam.seq_id==82U && trailer->bank_idx==104UL );
    }
    test_pack_callbacks_delete( e );
  }
}
...
static void
test_pack_tile_bam_ordinal_wrap_tracks_every_dispatched_batch( void ) {
  test_pack_callbacks_t e[1];
  test_pack_callbacks_new( e, FD_PACK_STRATEGY_BALANCED );
  fd_pack_ctx_t * ctx = e->h->ctx;
  test_pack_callbacks_insert( e, 42U, 200UL, 0 );

  /* Buffer future work before leadership while a second batch is replaced by
     same-identity resends.  BAM deliberately bypasses prepack signature
     deduplication, and replacing the second batch never empties Pack's
     bundle treap.  Prepare its resolved bytes once for the resend loop. */
  fd_txn_p_t invalid[1], txn[1];
  test_pack_tile_make_d18_poc_txns( invalid, txn );
  test_pack_tile_fill_sig( txn->payload+1UL, 43U );
  txn->payload[ TXN(txn)->acct_addr_off ] ^= 43U;
  uchar resolved[ FD_TPU_RESOLVED_MTU ] __attribute__((aligned(FD_CHUNK_ALIGN)));
  ulong sz = test_pack_tile_prepare_resolv_frag( e->h, resolved, txn->payload, txn->payload_sz,
                                                FD_TXN_M_TPU_SOURCE_BAM, 100UL );
  ctx->leader_slot = ULONG_MAX;
  fd_txn_m_t * m = (fd_txn_m_t *)resolved;
  m->bam.max_schedule_slot = 200UL;
  m->bam.seq_id = 43U;
  m->bam.scheduler_gen = 9U;
  m->bam.ownership_gen = ctx->bam_ownership_gen;
  m->bam.txn_cnt = 1U;

  /* BUNDLE_N in fd_pack.c is 313721.  Together with the retained first
     insertion, these resends cross its relative bundle ordinal wrap. */
  for( ulong i=0UL; i<313721UL; i++ ) {
    during_frag( ctx, 0UL, 0UL, 100UL, 0UL, sz, 0UL );
    after_frag( ctx, 0UL, 0UL, 100UL, sz, 0UL, 0UL, &e->h->out->stem );
    if( !(i & 1023UL) ) test_pack_callbacks_step( e );
  }
  FD_TEST( !test_pack_callbacks_dispatch_count( e ) );
  FD_TEST( ctx->bam_work_cnt==2UL && pack_tile_bam_pending_work_cnt( ctx )==2UL );
  FD_TEST( fd_pack_avail_txn_cnt( ctx->pack )==2UL && !ctx->bam_pending_result_cnt );

  test_pack_callbacks_leader( e, 200UL, 0 );
  for( int i=0; i<4 && (ctx->drain_execle || fd_pack_avail_txn_cnt( ctx->pack )); i++ )
    test_pack_callbacks_step( e );
  ...
  ulong txn_cnt = 0UL;
  for( ulong worker=0UL; worker<2UL; worker++ ) {
    ulong out_idx = worker ? 3UL : 0UL;
    for( ulong seq=0UL; seq<e->h->out->seqs[out_idx]; seq++ ) {
      ...
      for( ulong j=0UL; j<count; j++ ) {
        ...
        FD_TEST( txnp->bam.seq_id==42U || txnp->bam.seq_id==43U );
        uint bit = 1U<<(txnp->bam.seq_id-42U);
        FD_TEST( !(seen & bit) );
        seen |= bit;
        fd_memcpy( signatures[txn_cnt], fd_txn_get_signatures( TXN(txnp), txnp->payload ), sizeof(fd_ed25519_sig_t) );
        FD_TEST( pack_tile_bam_work_find( ctx, signatures[txn_cnt], 1, NULL )<ctx->bam_scheduled_work_cnt );
        txn_cnt++;
      }
    }
  }
  FD_TEST( txn_cnt==2UL && seen==3U );
  for( ulong i=0UL; i<txn_cnt; i++ )
    test_pack_tile_send_executed_txn( e->h, signatures[i], FD_EXECUTED_TXN_KIND_BAM_COMPLETED_UNLANDED );
  FD_TEST( !ctx->bam_work_cnt && !ctx->bam_scheduled_work_cnt && !ctx->bam_pending_result_cnt );
  FD_TEST( ctx->bam_work_item_stage_cnt[ FD_METRICS_ENUM_PACK_BAM_WORK_STAGE_V_COMPLETED_UNLANDED_IDX ]==2UL );
  test_pack_callbacks_delete( e );
}

/* Exercise both old slot-age eviction sites with actual pack storage, and
   keep pending work across unrelated newer resolver observations.  Such
   observations must not advance BAM's target-slot admission floor. */
static void
test_pack_tile_insert_pending_single( fd_pack_t *       pack,
                                      fd_txn_p_t const * txnp,
                                      ulong             reference_slot ) {
  fd_txn_e_t * slot = fd_pack_insert_txn_init( pack );
  *slot->txnp = *txnp;
  ulong deleted;
  FD_TEST( fd_pack_insert_txn_fini( pack, slot, reference_slot, &deleted )>=0 && !deleted );
}

/* Resolver reference progress and leader entry are the two tile-owned
   retention clocks.  A signed nonce and BAM batch must survive both, while
   ordinary work immediately below each cutoff leaves the real Pack heap. */
static void
test_pack_tile_retention_callbacks( void ) {
  test_pack_callbacks_t e[1];
  test_pack_callbacks_new( e, FD_PACK_STRATEGY_BALANCED );
  fd_pack_ctx_t * ctx = e->h->ctx;
  fd_txn_p_t invalid[1], ordinary[1], nonce[1];
  test_pack_tile_make_d18_poc_txns( invalid, ordinary );
  test_pack_tile_make_d26_poc_txn( nonce, 1 );

  test_pack_callbacks_insert_at( e, 88U, 501UL, 0, 10UL );
  FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==1UL );
  test_pack_tile_insert_pending_single( ctx->pack, nonce, 99UL );
  test_pack_tile_insert_pending_single( ctx->pack, ordinary, 99UL );
  FD_TEST( fd_pack_avail_txn_cnt( ctx->pack )==3UL );

  uchar resolved[ FD_TPU_RESOLVED_MTU ] __attribute__((aligned(FD_CHUNK_ALIGN)));
  ulong sz = test_pack_tile_prepare_resolv_frag( e->h, resolved, ordinary->payload,
                                                  ordinary->payload_sz, FD_TXN_M_TPU_SOURCE_QUIC, 400UL );
  ctx->leader_slot = ULONG_MAX;
  ulong expired_before = FD_MCNT_GET( PACK, TXN_EXPIRED );
  during_frag( ctx, 0UL, 0UL, 400UL, 0UL, sz, 0UL );
  after_frag( ctx, 0UL, 0UL, 400UL, sz, 0UL, 0UL, &e->h->out->stem );
  FD_TEST( ctx->highest_observed_slot==400UL && ctx->leader_slot==ULONG_MAX );
  FD_TEST( FD_MCNT_GET( PACK, TXN_EXPIRED )==expired_before+1UL );
  FD_TEST( fd_pack_avail_txn_cnt( ctx->pack )==2UL && pack_tile_bam_pending_work_cnt( ctx )==1UL );

  test_pack_tile_insert_pending_single( ctx->pack, ordinary, 200UL );
  FD_TEST( fd_pack_avail_txn_cnt( ctx->pack )==3UL );
  test_pack_callbacks_leader( e, 501UL, 0 );
  FD_TEST( FD_MCNT_GET( PACK, TXN_EXPIRED )==expired_before+2UL );
  FD_TEST( fd_pack_avail_txn_cnt( ctx->pack )==2UL && pack_tile_bam_pending_work_cnt( ctx )==1UL );
  FD_TEST( fd_pack_delete_transaction( ctx->pack,
             fd_txn_get_signatures( TXN(nonce), nonce->payload ) )==1UL );
  FD_TEST( fd_pack_avail_txn_cnt( ctx->pack )==1UL );
  FD_TEST( fd_pack_contains_bam_bundle( ctx->pack,
             (fd_ed25519_sig_t const *)(void const *)ctx->bam_work[0].sig[0], 88U, 9U, 1 ) );
  test_pack_callbacks_delete( e );
}

static void
test_pack_tile_bam_blockhash_slot_is_not_validity( void ) {
  for( int frank=0; frank<2; frank++ ) {
    for( int revert=0; revert<2; revert++ ) {
      test_pack_callbacks_t e[1];
      test_pack_callbacks_new( e, FD_PACK_STRATEGY_BALANCED );
      ...
      test_pack_callbacks_insert_at( e, 71U, 400UL, revert, 10UL );
      FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==1UL );
      test_pack_callbacks_insert_at( e, 72U, 400UL, revert, 1000UL );
      FD_TEST( ctx->highest_observed_slot==1000UL );
      FD_TEST( ctx->leader_slot==ULONG_MAX );
      FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==2UL );
      FD_TEST( fd_pack_avail_txn_cnt( ctx->pack )==2UL );
      FD_TEST( !ctx->bam_pending_result_cnt );
      /* Same observed history, actual execution bank at slot 400.  The
         old known hash and unknown-hash fallback must both survive. */
      test_pack_callbacks_leader( e, 400UL, frank );
      FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==2UL );
      FD_TEST( fd_pack_avail_txn_cnt( ctx->pack )==2UL );
      test_pack_callbacks_insert_at( e, 73U, 400UL, revert, 0UL );
      FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==3UL );
      FD_TEST( fd_pack_avail_txn_cnt( ctx->pack )==3UL );
      FD_TEST( !ctx->bam_pending_result_cnt );
      test_pack_callbacks_step( e );
      FD_TEST( test_pack_callbacks_dispatch_count( e )==1UL );
      fd_microblock_execle_trailer_t const * trailer;
      fd_txn_e_t const * txn = test_pack_callbacks_output( e, 0UL, &trailer );
      FD_TEST( txn->txnp->bam.seq_id==71U );
      FD_TEST( trailer->bank_idx==400UL );
      /* Closing the actual target still retires all unscheduled members. */
      pack_tile_finish_leader_slot( ctx, &e->h->out->stem, fd_tickcount(), "test",
          FD_PACK_END_SLOT_REASON_TIME, PACK_TILE_BAM_BUNDLE_ASSEMBLY_ABANDON_POH_TIMEOUT );
      FD_TEST( ctx->bam_min_admission_slot==401UL );
      FD_TEST( !pack_tile_bam_pending_work_cnt( ctx ) );
      FD_TEST( !fd_pack_avail_txn_cnt( ctx->pack ) );
      test_pack_callbacks_delete( e );
    }
  }
...
static void
test_pack_tile_bam_closed_admission_callbacks( void ) {
  for( int frank=0; frank<2; frank++ ) {
    test_pack_callbacks_t e[1];
    test_pack_callbacks_new( e, FD_PACK_STRATEGY_BALANCED );
    fd_pack_ctx_t * ctx = e->h->ctx;
    test_pack_callbacks_leader( e, 104UL, frank );
    ctx->highest_observed_slot = 100UL;
    pack_tile_finish_leader_slot( ctx, &e->h->out->stem, fd_tickcount(), "test",
        FD_PACK_END_SLOT_REASON_TIME, PACK_TILE_BAM_BUNDLE_ASSEMBLY_ABANDON_POH_TIMEOUT );
    FD_TEST( ctx->bam_min_admission_slot==105UL && ctx->leader_slot==ULONG_MAX );
    test_pack_callbacks_insert( e, 61U, 104UL, 0 );
    FD_TEST( !pack_tile_bam_pending_work_cnt( ctx ) && ctx->bam_pending_result_cnt==1UL );
    test_pack_callbacks_insert( e, 62U, 105UL, 1 );
    FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==1UL );
    FD_TEST( ctx->highest_observed_slot==100UL );
    /* Scheduling floors remain authoritative even though observed hash
       slots no longer constrain validity.  Unknown current slots are explicit. */
    FD_TEST( pack_tile_bam_target_expired( ctx, 104UL ) );
    FD_TEST( !pack_tile_bam_target_expired( ctx, 105UL ) );
    ulong closed_outputs = e->h->out->seqs[1];
    pack_tile_finish_leader_slot( ctx, &e->h->out->stem, fd_tickcount(), "unknown",
        FD_PACK_END_SLOT_REASON_TIME, PACK_TILE_BAM_BUNDLE_ASSEMBLY_ABANDON_POH_TIMEOUT );
    FD_TEST( ctx->bam_min_admission_slot==105UL && e->h->out->seqs[1]==closed_outputs );
    /* An unexpected backward bank does not reopen the closed slot. */
    test_pack_callbacks_leader( e, 103UL, frank );
    test_pack_callbacks_insert( e, 63U, 104UL, 0 );
    FD_TEST( pack_tile_bam_pending_work_cnt( ctx )==1UL && ctx->bam_min_admission_slot==105UL );
    /* Skip the target: rejection retains its original slot. */
    test_pack_callbacks_leader( e, 106UL, frank );
    FD_TEST( !pack_tile_bam_pending_work_cnt( ctx ) );
    FD_TEST( !test_pack_callbacks_dispatch_count( e ) );
    test_pack_callbacks_delete( e );
  }
...
static void
test_pack_tile_bam_result_drain_before_skip( void ) {
  test_pack_callbacks_t e[1];
  test_pack_callbacks_new( e, FD_PACK_STRATEGY_BALANCED );
  fd_pack_ctx_t * ctx = e->h->ctx;
  ctx->drain_execle = 1;
  ctx->execle_idle_bitset = 0UL;
  /* after_credit runs only when every output has STEM_BURST credits.
     Exercise real credit consumption, including an undrained FIFO tail
     that must survive until the result consumer returns credit. */
  for( ulong i=0UL; i<4UL; i++ ) e->h->out->cr_avail[i] = TEST_PACK_TILE_MCACHE_DEPTH;
  e->h->out->cr_avail[2] = 4UL;
  e->h->out->min_cr_avail = 4UL;
  e->h->out->stem.cr_decrement_amount = 1UL;
  fd_bam_bundle_result_t result = fd_bam_result_base( 70U, 9U, 104UL, 1U );
  for( int i=0; i<4; i++ ) {
    result.seq_id = (uint)(70+i);
    FD_TEST( pack_tile_enqueue_bam_result( ctx, &result ) );
  }
  ctx->skip_cnt = 100L;
  for( ulong i=1UL; i<=3UL; i++ ) {
    FD_TEST( e->h->out->min_cr_avail>=2UL ); /* Pack's STEM_BURST */
    int poll=1, busy=0;
    after_credit( ctx, &e->h->out->stem, &poll, &busy );
    FD_TEST( busy && e->h->out->seqs[2]==i && ctx->bam_pending_result_cnt==4UL-i );
    FD_TEST( e->h->out->cr_avail[2]==4UL-i && e->h->out->min_cr_avail==4UL-i );
    fd_frag_meta_t const * meta = &e->h->out->mcaches[2][i-1UL];
    fd_bam_bundle_result_t const * published = fd_chunk_to_laddr_const( e->h->out->dcache, meta->chunk );
    FD_TEST( published->seq_id==(uint)(69UL+i) && published->scheduler_gen==9U && published->slot==104UL );
    FD_TEST( ctx->drain_execle );
    FD_TEST( !test_pack_callbacks_dispatch_count( e ) && !e->h->out->seqs[1] );
  }
  FD_TEST( e->h->out->min_cr_avail<2UL && ctx->bam_pending_result_cnt==1UL );
  FD_TEST( ctx->bam_result_queue[ctx->bam_result_queue_head].seq_id==73U );
  /* Stem resumes the credited callback once the consumer advances. */
  e->h->out->cr_avail[2] = e->h->out->min_cr_avail = 2UL;
  int poll=1, busy=0;
  after_credit( ctx, &e->h->out->stem, &poll, &busy );
  FD_TEST( busy && !ctx->bam_pending_result_cnt && e->h->out->seqs[2]==4UL );
  FD_TEST( e->h->out->cr_avail[2]==1UL && e->h->out->min_cr_avail==1UL );
  FD_TEST( ctx->drain_execle );
  FD_TEST( !test_pack_callbacks_dispatch_count( e ) && !e->h->out->seqs[1] );
  test_pack_callbacks_delete( e );
}
...
test_pack_callbacks_initializer( test_pack_callbacks_t * e,
  ...
  test_pack_tile_fill_sig( bundle[0]->txnp->payload+1UL, seed );
  bundle[0]->txnp->payload[ TXN(bundle[0]->txnp)->acct_addr_off ] ^= seed;
  bundle[0]->txnp->source_tpu = FD_TXN_M_TPU_SOURCE_BUNDLE;
  fd_memcpy( ctx->crank->last_sig, bundle[0]->txnp->payload+1UL, sizeof(fd_ed25519_sig_t) );
  ulong deleted;
  FD_TEST( fd_pack_insert_bundle_fini( ctx->pack, bundle, 1UL, 100UL,
              FD_PACK_IB_TYPE_BAM, NULL, &deleted, NULL )>=0 );
  ctx->crank->enabled = 1;
...
test_pack_tile_bam_initializer_generation_callbacks( void ) {
  ...
  for( int scenario=0; scenario<5; scenario++ ) {
    ...
    if( scenario==1 ) {
      ...
      fd_pack_get_block_limits( ctx->pack, NULL, limits );
      ulong old_bytes = limits->max_data_bytes_per_block;
      /* Enough bytes for the scheduler's minimum-transaction guard, but
         too few for this 223-byte initializer plus its microblock header. */
      limits->max_data_bytes_per_block = 48UL+FD_TXN_MIN_SERIALIZED_SZ;
      ...
      for( ulong j=0UL; j<128UL; j++ ) {
        ...
        FD_TEST( !fd_pack_schedule_next_microblock_with_bundle_hint( ctx->pack, CUS_PER_MICROBLOCK, 0.0f, 0UL,
                    FD_PACK_SCHEDULE_BUNDLE | FD_PACK_SCHEDULE_BAM_ONLY | FD_PACK_SCHEDULE_BAM_READY, hint, NULL, NULL, output ) );
      }
      ulong deferred_hint;
    ...
    FD_TEST( e->generation==(8UL<<1) && ctx->bam_ownership_gen==8U );
    FD_TEST( e->status==FD_BAM_STATUS_FSEQ_OVERRIDE_ACTIVE && ctx->bam_override_snapshot );
    FD_TEST( !pack_tile_bam_pending_work_cnt( ctx ) );
    if( scenario<2 ) {
      FD_TEST( !fd_pack_avail_txn_cnt( ctx->pack ) && !ctx->bam_ib_associated && !ctx->crank->ib_inserted );
    ...
    if( scenario==2 || scenario==3 ) {
      test_pack_callbacks_step( e );
      FD_TEST( test_pack_callbacks_dispatch_count( e )==1UL && pack_tile_bam_pending_work_cnt( ctx )==1UL );
      /* Only the real initializer rebate releases Pending.  Failed keeps
         the crank peek NULL but permits best-effort BAM scheduling. */
      ctx->rebate->rebate->ib_result = scenario==2 ? 1 : 2;
      ctx->in_kind[0] = IN_KIND_EXECLE;
      after_frag( ctx, 0UL, 0UL, 104UL, 0UL, 0UL, 0UL, &e->h->out->stem );
    }
    test_pack_callbacks_step( e );
    FD_TEST( ctx->bam_scheduled_work_cnt==1UL && !pack_tile_bam_pending_work_cnt( ctx ) );
    fd_microblock_execle_trailer_t const * trailer;
    fd_txn_e_t const * txn = test_pack_callbacks_output( e, (ulong)(scenario>=2), &trailer );
    FD_TEST( txn->txnp->bam.seq_id==82U && trailer->bank_idx==104UL );
    test_pack_callbacks_delete( e );
  }
}
...
static void
test_pack_tile_bam_readiness_identity_and_mode( void ) {
  test_pack_callbacks_t e[1];
  test_pack_callbacks_new( e, FD_PACK_STRATEGY_BALANCED );
  fd_pack_ctx_t * ctx = e->h->ctx;
  test_pack_callbacks_leader( e, 104UL, 0 );
  ...
  FD_TEST( ctx->bam_candidate_identity_mismatch_cnt>=3UL );
  test_pack_callbacks_step( e );
  FD_TEST( test_pack_callbacks_dispatch_count( e )==1UL );
  test_pack_callbacks_delete( e );
}
...
static void
test_pack_tile_bam_expired_head_refreshes_crank_readiness( void ) {
  test_pack_callbacks_t e[1];
  test_pack_callbacks_new( e, FD_PACK_STRATEGY_BALANCED );
  fd_pack_ctx_t * ctx = e->h->ctx;
  test_pack_callbacks_leader( e, 104UL, 0 );
  test_pack_callbacks_insert( e, 95U, 104UL, 0 );
  /* Construct a stale head without the normal leader-start sweep: a
     slot-105 batch is buffered while no active leader is known. */
  ctx->leader_slot = ULONG_MAX;
  test_pack_callbacks_insert( e, 96U, 105UL, 0 );
  ctx->leader_slot = 104UL;
  /* Isolate the final dispatch guard's eviction path, without the normal
  ...
  fd_microblock_execle_trailer_t const * trailer;
  fd_txn_e_t const * txn = test_pack_callbacks_output( e, 0UL, &trailer );
  FD_TEST( txn->txnp->bam.seq_id==96U && trailer->bank_idx==105UL );
  test_pack_callbacks_delete( e );
}

static void
test_pack_tile_bam_partial_close_reports_outside_slot( void ) {
  test_pack_callbacks_t e[1];
  test_pack_callbacks_new( e, FD_PACK_STRATEGY_BALANCED );
  fd_pack_ctx_t * ctx = e->h->ctx;
  test_pack_callbacks_leader( e, 104UL, 0 );
  test_pack_tile_send_bam_resolv_frag( e->h, 101U, 9U, 7U, 104UL, 0U, 2U, 1, 104UL, 101U );
  FD_TEST( ctx->current_bundle->txn_received==1UL && !ctx->bam_pending_result_cnt );
  pack_tile_finish_leader_slot( ctx, &e->h->out->stem, fd_tickcount(), "partial-close",
      FD_PACK_END_SLOT_REASON_TIME, PACK_TILE_BAM_BUNDLE_ASSEMBLY_ABANDON_POH_TIMEOUT );
  FD_TEST( ctx->bam_min_admission_slot==105UL && !ctx->bam_pending_result_cnt );
  test_pack_tile_send_bam_resolv_frag( e->h, 101U, 9U, 7U, 104UL, 1U, 2U, 1, 100UL, 102U );
  FD_TEST( ctx->bam_pending_result_cnt==1UL && !pack_tile_bam_pending_work_cnt( ctx ) );
  fd_bam_bundle_result_t const * res = &ctx->bam_result_queue[ ctx->bam_result_queue_head ];
  FD_TEST( res->seq_id==101U && res->slot==104UL );
  FD_TEST( res->scheduling_error==FD_BAM_SCHED_ERR_OUTSIDE_SLOT && !res->transaction_err_count );
  test_pack_callbacks_delete( e );
}


static void
test_pack_tile_bam_unscheduled_metric( void ) {
  test_pack_callbacks_t e[1];
  test_pack_callbacks_new( e, FD_PACK_STRATEGY_BALANCED );
  test_pack_callbacks_leader( e, 104UL, 0 );
  test_pack_callbacks_insert( e, 41U, 104UL, 0 );
  test_pack_callbacks_step( e );
  FD_TEST( e->h->ctx->bam_scheduled_work_cnt==1UL );
  test_pack_callbacks_insert( e, 42U, 104UL, 0 );
  ulong before = FD_MCNT_GET( PACK, BAM_UNSCHEDULED_AT_SLOT_END );
  pack_tile_finish_leader_slot( e->h->ctx, &e->h->out->stem, fd_tickcount(), "metric-test",
      FD_PACK_END_SLOT_REASON_TIME, PACK_TILE_BAM_BUNDLE_ASSEMBLY_ABANDON_POH_TIMEOUT );
  FD_TEST( FD_MCNT_GET( PACK, BAM_UNSCHEDULED_AT_SLOT_END )==before+1UL );
  test_pack_callbacks_insert( e, 43U, 105UL, 0 );
  FD_TEST( pack_tile_bam_pending_work_cnt( e->h->ctx )==1UL );
  pack_tile_finish_leader_slot( e->h->ctx, &e->h->out->stem, fd_tickcount(), "metric-test",
      FD_PACK_END_SLOT_REASON_TIME, PACK_TILE_BAM_BUNDLE_ASSEMBLY_ABANDON_POH_TIMEOUT );
  FD_TEST( FD_MCNT_GET( PACK, BAM_UNSCHEDULED_AT_SLOT_END )==before+1UL );

  /* Backward leader notifications cannot reopen the closed floor or count
     its future work.  Skipping a target evicts it at leader entry, not close. */
  test_pack_callbacks_leader( e, 103UL, 0 );
  pack_tile_finish_leader_slot( e->h->ctx, &e->h->out->stem, fd_tickcount(), "metric-backward",
      FD_PACK_END_SLOT_REASON_TIME, PACK_TILE_BAM_BUNDLE_ASSEMBLY_ABANDON_POH_TIMEOUT );
  FD_TEST( FD_MCNT_GET( PACK, BAM_UNSCHEDULED_AT_SLOT_END )==before+1UL );
  FD_TEST( pack_tile_bam_pending_work_cnt( e->h->ctx )==1UL );
  test_pack_callbacks_leader( e, 106UL, 0 );
  FD_TEST( !pack_tile_bam_pending_work_cnt( e->h->ctx ) );
  test_pack_callbacks_insert( e, 44U, 106UL, 0 );
  e->h->ctx->in_kind[0] = IN_KIND_REPLAY;
  after_frag( e->h->ctx, 0UL, 0UL, REPLAY_SIG_RESET, 0UL, 0UL, 0UL, &e->h->out->stem );
  FD_TEST( FD_MCNT_GET( PACK, BAM_UNSCHEDULED_AT_SLOT_END )==before+2UL );
  test_pack_callbacks_insert( e, 45U, 109UL, 0 );
  FD_TEST( pack_tile_bam_pending_work_cnt( e->h->ctx )==1UL );

  /* Ownership retirement removes pending work before a later slot closes. */
  test_pack_callbacks_leader( e, 109UL, 0 );
  e->generation = (8UL<<1) | 1UL;
  pack_tile_sync_bam_ownership_generation( e->h->ctx );
  FD_TEST( !pack_tile_bam_pending_work_cnt( e->h->ctx ) );
  pack_tile_finish_leader_slot( e->h->ctx, &e->h->out->stem, fd_tickcount(), "metric-ownership",
      FD_PACK_END_SLOT_REASON_TIME, PACK_TILE_BAM_BUNDLE_ASSEMBLY_ABANDON_POH_TIMEOUT );
  FD_TEST( FD_MCNT_GET( PACK, BAM_UNSCHEDULED_AT_SLOT_END )==before+2UL );
  test_pack_callbacks_delete( e );
}
...
main( int     argc,
  ...
  test_pack_tile_bam_unscheduled_metric();

  test_pack_tile_bam_target_slot_callbacks();
  test_pack_tile_bam_secondary_callbacks();
  test_pack_tile_bam_conflict_lookahead_callbacks();
  test_pack_tile_bam_ordinal_wrap_tracks_every_dispatched_batch();
  test_pack_tile_retention_callbacks();
  test_pack_tile_bam_blockhash_slot_is_not_validity();
  test_pack_tile_bam_closed_admission_callbacks();
  test_pack_tile_bam_result_drain_before_skip();
  test_pack_tile_bam_initializer_generation_callbacks();
  test_pack_tile_bam_readiness_identity_and_mode();
  test_pack_tile_bam_partial_close_reports_outside_slot();
  test_pack_tile_bam_expired_head_refreshes_crank_readiness();
  ...
  test_pack_tile_bam_old_member_does_not_override_missed_target();
  ...
  test_pack_tile_bam_unsigned_advance_passes_pack();
