...
fd_bam_validate_batch( fd_bam_tile_t *                  ctx,
  ...
  if( FD_UNLIKELY( ctx->bam_leader_state.slot!=ULONG_MAX &&
    ...
    fd_bam_bundle_result_t res = fd_bam_result_base( batch->seq_id, ctx->scheduler_gen, batch->max_schedule_slot, state->packet_cnt );
  ...
  if( FD_UNLIKELY( state->packet_cnt == 0U ) ) {
    ...
    res.deser_reason = bam_types_DeserializationErrorReason_EMPTY;
    fd_bam_enqueue_result( ctx, &res );
    return 0;
  }
  ...
  for( uchar i=0U; i<state->packet_cnt; i++ ) {
    ...
    if( FD_LIKELY( !fd_txn_is_simple_vote_transaction( (fd_txn_t const *)parsed->txn_t[ i ], packet->payload ) ) ) continue;
    ctx->metrics.ingress_batch_rejected_cnt[ FD_METRICS_ENUM_BAM_INGRESS_BATCH_REJECT_REASON_V_VOTE_TRANSACTION_IDX ]++;
    fd_bam_bundle_result_t res = fd_bam_result_base( batch->seq_id, ctx->scheduler_gen, batch->max_schedule_slot, state->packet_cnt );
    res.bundle_err   = FD_BAM_BUNDLE_ERR_DESER;
    res.deser_reason = bam_types_DeserializationErrorReason_VOTE_TRANSACTION_FAILURE;
    res.deser_index  = i;
    fd_bam_enqueue_result( ctx, &res );
    return 0;
  }
...
fd_bam_publish_batch( fd_bam_tile_t *            ctx,
  ...
  if( FD_UNLIKELY( bam_pending_txn_avail( ctx->pending_txns ) < (ulong)packet_cnt ) ) {
    ...
    fd_bam_bundle_result_t res = fd_bam_result_base( batch->seq_id, ctx->scheduler_gen, batch->max_schedule_slot, packet_cnt );
...
#define FD_BAM_MULTI_DECODE_FAIL() do {                                                                    \
  ...
  while( pb_decode_tag( stream, &wire_type, &tag, &eof ) ) {
    ...
    if( (identity.has_seq_id || identity_ok) && state->has_deser_err ) {
      fd_bam_bundle_result_t res = fd_bam_result_base( identity.seq_id, ctx->scheduler_gen, identity.max_schedule_slot, state->packet_cnt );
      res.bundle_err   = FD_BAM_BUNDLE_ERR_DESER;
      res.deser_reason = state->deser_reason;
      res.deser_index  = state->deser_index;
      fd_bam_enqueue_result( ctx, &res );
    } else {
