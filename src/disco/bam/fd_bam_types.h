...
/* FD_BAM_RESULTS_PER_MESSAGE is how many queued results are packed into a
   single MultipleAtomicTxnBatchResult.  BAM's validator service accepts at
   most 24 results per message, matching Jito-Solana's outbound batch limit.
   The gRPC client only accepts one message per TX ring drain, so the drain
   rate is messages/second, not results/second; batching multiplies result
   throughput by this factor. */
#define FD_BAM_RESULTS_PER_MESSAGE 24U
