...
/* Deterministic clock for harness + fuzz reproducibility. */
...
static size_t
bam_e2e_encode_scheduler_heartbeat( ulong time_sent_microseconds,
                                    uchar * out,
                                    size_t  out_sz ) {
  bam_api_SchedulerResponse resp = bam_api_SchedulerResponse_init_default;
  resp.which_versioned_msg = bam_api_SchedulerResponse_v0_tag;
...
static size_t
bam_e2e_encode_auth_challenge( char const * challenge,
                               uchar *      out,
                               size_t       out_sz ) {
...
static size_t
bam_e2e_encode_config( uint commission_bps,
...
typedef struct {
  ...
  uchar idx;
} bam_e2e_wire_result_t;

FD_FN_UNUSED static _Bool
bam_e2e_decode_last_scheduler_message( fd_bam_tile_t *          state,
...
FD_FN_UNUSED static _Bool
bam_e2e_decode_last_wire_result( fd_bam_tile_t *         state,
                                 bam_e2e_wire_result_t * out ) {
  fd_grpc_client_t * client = state->grpc_client;
  fd_grpc_hdr_t hdr;
  fd_memcpy( &hdr, client->nanopb_tx, sizeof(fd_grpc_hdr_t) );
  uint msg_sz = fd_uint_bswap( hdr.msg_sz );
...
/* ---------- Synthetic pipeline model ---------- */

typedef enum {
  BAM_E2E_TXN_OK = 0,
  BAM_E2E_TXN_VERIFY_SIG_FAIL = 1,
  BAM_E2E_TXN_SANITIZE_FAIL = 2,
  BAM_E2E_TXN_LUT_FAIL = 3,
  BAM_E2E_TXN_BANK_FRONT_RUN_FAIL = 4,
  BAM_E2E_TXN_LOCK_FAIL = 5,
  BAM_E2E_TXN_EXEC_FAIL = 6,
  BAM_E2E_TXN_POH_TIMEOUT = 7,
  BAM_E2E_TXN_DUPLICATE = 8,
} bam_e2e_txn_mode_t;
...
typedef struct {
  ...
  uchar revert_on_error;
  uchar txn_cnt;
  bam_e2e_stage_txn_t txn[ FD_PACK_MAX_TXN_PER_BUNDLE ];
} bam_e2e_batch_t;
...
typedef enum {
  ...
  BAM_TRACE_PACK   = 3,
  BAM_TRACE_BANK   = 4,
} bam_e2e_trace_stage_t;

typedef struct {
  bam_e2e_trace_stage_t stage;
  uint  seq_id;
  uchar batch_idx;
  uchar batch_cnt;
  uchar revert_on_error;
...
#define BAM_E2E_MAX_READY_BATCHES 1024UL
#define BAM_E2E_MAX_RESULTS       4096UL
#define BAM_E2E_MAX_TRACES        16384UL
#define BAM_E2E_MAX_INTENTS       4096UL
...
typedef struct {
  ...
  /* staging */
  bam_e2e_partial_batch_t partial[1];
  bam_e2e_batch_t ready[ BAM_E2E_MAX_READY_BATCHES ];
  ulong ready_cnt;

  bam_e2e_intent_t intents[ BAM_E2E_MAX_INTENTS ];
  ulong intent_cnt;

  fd_bam_bundle_result_t sut_results[ BAM_E2E_MAX_RESULTS ];
  ulong sut_result_cnt;

  fd_bam_bundle_result_t oracle_results[ BAM_E2E_MAX_RESULTS ];
  ulong oracle_result_cnt;

  bam_e2e_trace_t traces[ BAM_E2E_MAX_TRACES ];
  ulong trace_cnt;

  bam_e2e_wire_result_t wire_results[ BAM_E2E_MAX_RESULTS ];
...
typedef struct {
  ...
  uchar revert_on_error;
  uchar txn_cnt;
  bam_e2e_txn_spec_t txn[ FD_PACK_MAX_TXN_PER_BUNDLE ];
} bam_e2e_batch_def_t;

static void
bam_e2e_trace( bam_e2e_harness_t *      h,
               bam_e2e_trace_stage_t    stage,
               bam_e2e_stage_txn_t const * txn ) {
  if( FD_UNLIKELY( h->trace_cnt >= BAM_E2E_MAX_TRACES ) ) return;
  bam_e2e_trace_t * tr = &h->traces[ h->trace_cnt++ ];
  tr->stage             = stage;
...
static inline ulong
bam_e2e_current_slot( bam_e2e_harness_t const * h ) {
  return h->leader_working_present ? h->leader_working_slot : h->bankforks_working_slot;
}

static void
bam_e2e_record_intent( bam_e2e_harness_t * h,
                       uint                 seq_id ) {
  ...
  if( FD_UNLIKELY( h->intent_cnt >= BAM_E2E_MAX_INTENTS ) ) return;
...
static void
bam_e2e_try_enqueue_result( bam_e2e_harness_t *            h,
...
static void
bam_e2e_emit_sut_result( bam_e2e_harness_t *               h,
                         fd_bam_bundle_result_t const *    res ) {
  bam_e2e_record_intent( h, res->seq_id );
  if( FD_LIKELY( h->sut_result_cnt < BAM_E2E_MAX_RESULTS ) ) h->sut_results[ h->sut_result_cnt++ ] = *res;
  bam_e2e_try_enqueue_result( h, res );
}

static void
bam_e2e_emit_oracle_result( bam_e2e_harness_t *               h,
                            fd_bam_bundle_result_t const *    res ) {
  if( FD_LIKELY( h->oracle_result_cnt < BAM_E2E_MAX_RESULTS ) ) h->oracle_results[ h->oracle_result_cnt++ ] = *res;
}

static bam_e2e_txn_spec_t
bam_e2e_parse_payload( uchar const * payload,
                       ulong         payload_sz ) {
  bam_e2e_txn_spec_t spec = {0};
  if( FD_UNLIKELY( payload_sz<5UL ) ) {
    spec.mode         = BAM_E2E_TXN_SANITIZE_FAIL;
...
static void
bam_e2e_prepare_scheduler_stream( fd_bam_tile_t * state ) {
...
static void
bam_e2e_keepalive_sync( fd_bam_tile_t * state,
                        long            now ) {
...
static void
bam_e2e_node_deliver_heartbeat( bam_e2e_harness_t * h ) {
  uchar pb[64];
  size_t pb_sz = bam_e2e_encode_scheduler_heartbeat( (ulong)fd_long_max( g_clock/1000L, 0L ), pb, sizeof(pb) );
  fd_bam_client_grpc_rx_msg( h->state, pb, pb_sz, FD_BAM_CLIENT_REQ_BAM_InitSchedulerStream );
}

static void
bam_e2e_node_push_config( bam_e2e_harness_t * h,
                          uint                 commission_bps ) {
  uchar pb[512];
  uchar pk[32];
  for( ulong i=0UL; i<32UL; i++ ) pk[i]=(uchar)(i+1U);
  size_t pb_sz = bam_e2e_encode_config( commission_bps, pk, pb, sizeof(pb) );
  fd_bam_client_grpc_rx_msg( h->state, pb, pb_sz, FD_BAM_CLIENT_REQ_BAM_GetBuilderConfig );
}

static void
bam_e2e_node_send_auth_challenge( bam_e2e_harness_t * h,
  ...
  uchar pb[256];
  size_t pb_sz = bam_e2e_encode_auth_challenge( challenge, pb, sizeof(pb) );
  h->state->bam_auth_inflight = 1U;
  fd_bam_client_grpc_rx_msg( h->state, pb, pb_sz, FD_BAM_CLIENT_REQ_BAM_GetAuthChallenge );
}

static void
bam_e2e_node_deliver_batches( bam_e2e_harness_t *      h,
                              bam_e2e_batch_def_t const * defs,
                              ulong                     def_cnt,
                              uchar                     drop_last_idx ) {
  ...
  for( ulong i=0UL; i<def_cnt; i++ ) {
    bam_e2e_batch_def_t const * def = &defs[ i ];
  ...
  uchar pb[4096];
  size_t pb_sz = bam_e2e_encode_scheduler_batches( batches, def_cnt, pb, sizeof(pb) );
  fd_bam_client_grpc_rx_msg( h->state, pb, pb_sz, FD_BAM_CLIENT_REQ_BAM_InitSchedulerStream );
}

static void
bam_e2e_set_partial( bam_e2e_partial_batch_t * p,
                     bam_e2e_stage_txn_t const * t ) {
...
static void
bam_e2e_emit_partial_missing( bam_e2e_harness_t * h,
                              bam_e2e_partial_batch_t * p ) {
  ...
  bam_e2e_emit_sut_result( h, &res );
  bam_e2e_emit_oracle_result( h, &res );
  p->active = 0U;
}

static void
bam_e2e_ready_push( bam_e2e_harness_t * h,
                    bam_e2e_batch_t const * b ) {
  if( FD_UNLIKELY( h->ready_cnt>=BAM_E2E_MAX_READY_BATCHES ) ) return;
  h->ready[ h->ready_cnt++ ] = *b;
}

static int
bam_e2e_batch_precheck_fail( bam_e2e_batch_t const * b,
                             ulong                   current_slot,
...
static void
bam_e2e_sort_ready_by_priority( bam_e2e_harness_t * h ) {
...
bam_e2e_execute_batch( bam_e2e_harness_t * h,
                       bam_e2e_batch_t const * b ) {
  fd_bam_bundle_result_t res = {0};
  res.seq_id            = b->seq_id;
  res.slot              = bam_e2e_current_slot( h );
  ...
  for( uchar i=0U; i<b->txn_cnt; i++ ) {
    bam_e2e_txn_spec_t const * spec = &b->txn[i].spec;
  ...
  for( uchar i=0U; i<b->txn_cnt; i++ ) {
    ...
    switch( (bam_e2e_txn_mode_t)spec->mode ) {
      case BAM_E2E_TXN_LOCK_FAIL:
  ...
  bam_e2e_txn_spec_t const * spec0 = &b->txn[0].spec;
...
static void
bam_e2e_process_ready( bam_e2e_harness_t * h ) {
  if( FD_UNLIKELY( !h->ready_cnt ) ) return;
  bam_e2e_sort_ready_by_priority( h );

  for( ulong i=0UL; i<h->ready_cnt; i++ ) {
    bam_e2e_batch_t const * b = &h->ready[ i ];
    ...
    if( FD_UNLIKELY( b->max_schedule_slot!=FD_BAM_MAX_SCHEDULE_SLOT_DEFAULT &&
                     ...
                     b->max_schedule_slot<bam_e2e_current_slot( h ) ) ) {
      res = (fd_bam_bundle_result_t){
        .seq_id            = b->seq_id,
        .slot              = b->max_schedule_slot,
        .bundle_txn_cnt    = b->txn_cnt,
...
static void
bam_e2e_absorb_verify_output( bam_e2e_harness_t * h ) {
  ...
  while( h->out_read_seq < published ) {
    ...
    bam_e2e_stage_txn_t t = {
      ...
      .spec              = bam_e2e_parse_payload( fd_txn_m_payload_const( txnm ), txnm->payload_sz ),
    };

    bam_e2e_trace( h, BAM_TRACE_VERIFY, &t );
    bam_e2e_trace( h, BAM_TRACE_DEDUP,  &t );
    bam_e2e_trace( h, BAM_TRACE_RESOLV, &t );

    bam_e2e_partial_batch_t * p = h->partial;

    if( FD_UNLIKELY( p->active && p->revert_on_error && p->seq_id!=t.seq_id ) ) {
      bam_e2e_emit_partial_missing( h, p );
    }
    ...
    if( FD_UNLIKELY( p->active && p->seq_id==t.seq_id && p->revert_on_error!=t.revert_on_error ) ) {
      ...
      bam_e2e_emit_sut_result( h, &rej );
      bam_e2e_emit_oracle_result( h, &rej );
      p->active = 0U;
      h->out_read_seq++;
      continue;
    }

    if( FD_UNLIKELY( !p->active ) ) bam_e2e_set_partial( p, &t );
    if( FD_UNLIKELY( p->seq_id!=t.seq_id ) ) bam_e2e_set_partial( p, &t );
    ...
    if( FD_UNLIKELY( t.batch_idx==0U ) ) {
      ...
      fd_bam_bundle_result_t rej;
      if( FD_UNLIKELY( bam_e2e_batch_precheck_fail( &tmp, bam_e2e_current_slot( h ), &rej ) ) ) {
        bam_e2e_emit_sut_result( h, &rej );
        bam_e2e_emit_oracle_result( h, &rej );
        p->active = 0U;
        h->out_read_seq++;
    ...
    if( FD_UNLIKELY( t.spec.mode==BAM_E2E_TXN_VERIFY_SIG_FAIL ||
                     t.spec.mode==BAM_E2E_TXN_SANITIZE_FAIL   ||
                     t.spec.mode==BAM_E2E_TXN_LUT_FAIL ) ) {
      fd_bam_bundle_result_t rej = {
        .seq_id            = t.seq_id,
        .slot              = t.max_schedule_slot,
        .bundle_txn_cnt    = t.batch_cnt,
    ...
    if( FD_UNLIKELY( t.spec.mode==BAM_E2E_TXN_BANK_FRONT_RUN_FAIL ) ) {
      fd_bam_bundle_result_t rej = {
        .seq_id            = t.seq_id,
        .slot              = t.max_schedule_slot,
        .bundle_txn_cnt    = t.batch_cnt,
    ...
    bam_e2e_trace( h, BAM_TRACE_PACK, &t );
    ...
    if( FD_UNLIKELY( complete ) ) {
      ...
      for( uchar j=0U; j<p->txn_cnt; j++ ) {
        b.txn[j] = p->txn[j];
        bam_e2e_trace( h, BAM_TRACE_BANK, &b.txn[j] );
      }
      bam_e2e_ready_push( h, &b );
      p->active = 0U;
    }
...
bam_e2e_flush_wire_results( bam_e2e_harness_t * h ) {
  while( h->state->bam_pending_results ) {
    FD_TEST( fd_bam_test_flush_results( h->state )==1 );
    if( FD_UNLIKELY( h->wire_result_cnt >= BAM_E2E_MAX_RESULTS ) ) continue;
    bam_e2e_wire_result_t * wr = &h->wire_results[ h->wire_result_cnt ];
    if( FD_UNLIKELY( bam_e2e_decode_last_wire_result( h->state, wr ) ) ) h->wire_result_cnt++;
  }
}

static void
bam_e2e_apply_pipeline( bam_e2e_harness_t * h ) {
  bam_e2e_absorb_verify_output( h );
  bam_e2e_process_ready( h );
}

static void
bam_e2e_keyguard_setup( bam_e2e_harness_t * h,
                        fd_wksp_t *         wksp ) {
...
static void
bam_e2e_keyguard_teardown( bam_e2e_harness_t * h ) {
...
static void
bam_e2e_init( bam_e2e_harness_t * h,
              fd_wksp_t *         wksp ) {
  fd_memset( h, 0, sizeof(*h) );
  test_bam_env_create( h->env, wksp );
  h->state = h->env->state;
  bam_e2e_keyguard_setup( h, wksp );

  test_bam_env_mock_conn( h->env );
  bam_e2e_prepare_scheduler_stream( h->state );
  g_clock = (long)9e9;
  bam_e2e_keepalive_sync( h->state, g_clock );
  ...
  bam_e2e_node_push_config( h, 300U );
  bam_e2e_node_deliver_heartbeat( h );
}

static void
bam_e2e_fini( bam_e2e_harness_t * h ) {
  bam_e2e_keyguard_teardown( h );
  test_bam_env_destroy( h->env );
}

static void
bam_e2e_assert_trace_metadata( bam_e2e_harness_t const * h ) {
  for( ulong i=0UL; i<h->trace_cnt; i++ ) {
    bam_e2e_trace_t const * tr = &h->traces[i];
...
static void
bam_e2e_assert_intents( bam_e2e_harness_t const * h ) {
  for( ulong i=0UL; i<h->intent_cnt; i++ ) {
...
static void
bam_e2e_assert_differential( bam_e2e_harness_t const * h ) {
  FD_TEST( h->sut_result_cnt == h->oracle_result_cnt );
  for( ulong i=0UL; i<h->sut_result_cnt; i++ ) {
    fd_bam_bundle_result_t const * a = &h->sut_results[i];
...
static bam_e2e_batch_def_t
bam_e2e_make_batch( uint  seq_id,
                    ulong max_schedule_slot,
                    uchar revert_on_error,
                    uchar txn_cnt ) {
  bam_e2e_batch_def_t def;
...
static void
bam_e2e_run_scenario_atomic_success( bam_e2e_harness_t * h ) {
  bam_e2e_batch_def_t b = bam_e2e_make_batch( 10U, 100UL, 1U, 3U );
  bam_e2e_node_deliver_batches( h, &b, 1UL, 0U );
  bam_e2e_apply_pipeline( h );

  FD_TEST( h->sut_result_cnt==1UL );
  FD_TEST( h->sut_results[0].execution_success==1U );
  FD_TEST( h->charged_txn_cnt==3UL );
}

static void
bam_e2e_run_scenario_prevalidation_too_many_packets( bam_e2e_harness_t * h ) {
  bam_e2e_batch_t b = {
    .seq_id            = 901U,
    .max_schedule_slot = bam_e2e_current_slot( h ),
    .revert_on_error   = 1U,
    .txn_cnt           = (uchar)( FD_PACK_MAX_TXN_PER_BUNDLE + 1U ),
  };
  fd_bam_bundle_result_t rej = {0};
  FD_TEST( bam_e2e_batch_precheck_fail( &b, bam_e2e_current_slot( h ), &rej ) );
...
static void
bam_e2e_run_scenario_prevalidation_inconsistent_revert( bam_e2e_harness_t * h ) {
  ...
  h->partial->seen[0]           = 1U;
  h->partial->txn[0] = (bam_e2e_stage_txn_t){
    .seq_id            = 902U,
  ...
  bam_e2e_batch_def_t b = bam_e2e_make_batch( 902U, 100UL, 0U, 1U );
  bam_e2e_node_deliver_batches( h, &b, 1UL, 0U );
  bam_e2e_apply_pipeline( h );

  FD_TEST( h->sut_result_cnt==1UL );
  FD_TEST( h->sut_results[0].seq_id       == 902U );
  FD_TEST( h->sut_results[0].bundle_err   == FD_BAM_BUNDLE_ERR_DESER );
  FD_TEST( h->sut_results[0].deser_reason == bam_types_DeserializationErrorReason_INCONSISTENT_BUNDLE );
  FD_TEST( h->sut_results[0].deser_index  == 0U );
}

static void
bam_e2e_run_scenario_prevalidation_stale_slot( bam_e2e_harness_t * h ) {
  h->leader_working_present = 1U;
  h->leader_working_slot    = 200UL;
  h->bankforks_working_slot = 150UL;

  bam_e2e_batch_def_t b = bam_e2e_make_batch( 903U, 199UL, 1U, 2U );
  bam_e2e_node_deliver_batches( h, &b, 1UL, 0U );
  bam_e2e_apply_pipeline( h );
...
bam_e2e_run_scenario_slot_source_fallback( bam_e2e_harness_t * h ) {
  ...
  bam_e2e_batch_def_t b = bam_e2e_make_batch( 904U, 139UL, 1U, 1U );
  bam_e2e_node_deliver_batches( h, &b, 1UL, 0U );
  bam_e2e_apply_pipeline( h );
...
static void
bam_e2e_run_scenario_non_leader_phase( bam_e2e_harness_t * h ) {
  bam_e2e_batch_def_t buffered = bam_e2e_make_batch( 905U, 100UL, 1U, 2U );
  bam_e2e_node_deliver_batches( h, &buffered, 1UL, 0U );
  bam_e2e_absorb_verify_output( h );
  ...
  bam_e2e_process_ready( h );
  FD_TEST( h->sut_result_cnt==1UL );
  FD_TEST( h->sut_results[0].seq_id==905U );
  FD_TEST( h->sut_results[0].scheduling_error==FD_BAM_SCHED_ERR_OUTSIDE_SLOT );

  /* Newly received work while non-leader should also map to OUTSIDE_SLOT. */
  bam_e2e_batch_def_t fresh = bam_e2e_make_batch( 906U, 100UL, 1U, 1U );
  bam_e2e_node_deliver_batches( h, &fresh, 1UL, 0U );
  bam_e2e_apply_pipeline( h );
  FD_TEST( h->sut_result_cnt==2UL );
  FD_TEST( h->sut_results[1].seq_id==906U );
  FD_TEST( h->sut_results[1].scheduling_error==FD_BAM_SCHED_ERR_OUTSIDE_SLOT );
}

static void
bam_e2e_run_scenario_bank_unavailable_timeout( bam_e2e_harness_t * h ) {
  h->bank_available = 0U;
  bam_e2e_batch_def_t b = bam_e2e_make_batch( 907U, 100UL, 1U, 2U );
  bam_e2e_node_deliver_batches( h, &b, 1UL, 0U );
  bam_e2e_apply_pipeline( h );

  FD_TEST( h->sut_result_cnt==1UL );
  FD_TEST( !h->sut_results[0].execution_success );
  FD_TEST( h->sut_results[0].scheduling_error==FD_BAM_SCHED_ERR_POH_TIMEOUT );
}

static void
bam_e2e_run_scenario_auth_and_control( bam_e2e_harness_t * h ) {
  char const challenge[] = "e2e-auth-challenge";
  bam_e2e_node_send_auth_challenge( h, challenge );
  ...
  FD_TEST( h->state->bam_challenge_to_sign_len==18U );
  ...
  *h->state->grpc_client->request_tx_op = (fd_h2_tx_op_t){0};
  ...
  FD_TEST( bam_e2e_decode_last_scheduler_message( h->state, &out_msg ) );
...
static void
bam_e2e_run_scenario_atomic_verify_fail( bam_e2e_harness_t * h ) {
  bam_e2e_batch_def_t b = bam_e2e_make_batch( 11U, 100UL, 1U, 2U );
  b.txn[0].mode = BAM_E2E_TXN_VERIFY_SIG_FAIL;
  bam_e2e_node_deliver_batches( h, &b, 1UL, 0U );
  bam_e2e_apply_pipeline( h );
...
static void
bam_e2e_run_scenario_atomic_lut_fail( bam_e2e_harness_t * h ) {
  bam_e2e_batch_def_t b = bam_e2e_make_batch( 12U, 100UL, 1U, 2U );
  b.txn[1].mode = BAM_E2E_TXN_LUT_FAIL;
  bam_e2e_node_deliver_batches( h, &b, 1UL, 0U );
  bam_e2e_apply_pipeline( h );

  FD_TEST( h->sut_result_cnt==1UL );
  FD_TEST( h->sut_results[0].bundle_err==FD_BAM_BUNDLE_ERR_DESER );
  FD_TEST( h->sut_results[0].deser_index==1U );
}

static void
bam_e2e_run_scenario_atomic_exec_fail( bam_e2e_harness_t * h ) {
  bam_e2e_batch_def_t b = bam_e2e_make_batch( 13U, 100UL, 1U, 3U );
  b.txn[1].mode = BAM_E2E_TXN_EXEC_FAIL;
  bam_e2e_node_deliver_batches( h, &b, 1UL, 0U );
  bam_e2e_apply_pipeline( h );

  FD_TEST( h->sut_result_cnt==1UL );
  FD_TEST( !h->sut_results[0].execution_success );
  FD_TEST( h->sut_results[0].transaction_err_count==3U );
  FD_TEST( h->sut_results[0].transaction_err[1]==bam_types_TransactionErrorReason_INSTRUCTION_ERROR );
  FD_TEST( h->sut_results[0].transaction_err[0]==bam_types_TransactionErrorReason_COMMIT_CANCELLED );
}

static void
bam_e2e_run_scenario_partial_then_seq_switch( bam_e2e_harness_t * h ) {
  bam_e2e_batch_def_t b0 = bam_e2e_make_batch( 14U, 100UL, 1U, 3U );
  bam_e2e_batch_def_t b1 = bam_e2e_make_batch( 15U, 100UL, 1U, 2U );
  ...
  for( uchar i=0U; i<2U; i++ ) {
    h->partial->seen[ i ] = 1U;
    h->partial->txn[ i ]  = (bam_e2e_stage_txn_t){
      .seq_id            = b0.seq_id,
  ...
  bam_e2e_node_deliver_batches( h, &b1, 1UL, 0U );
  bam_e2e_apply_pipeline( h );

  fd_bam_bundle_result_t const * seq14 = NULL;
  fd_bam_bundle_result_t const * seq15 = NULL;
  for( ulong i=0UL; i<h->sut_result_cnt; i++ ) {
    if( h->sut_results[i].seq_id==14U ) seq14 = &h->sut_results[i];
    if( h->sut_results[i].seq_id==15U ) seq15 = &h->sut_results[i];
  }
...
static void
bam_e2e_run_scenario_non_atomic_success_fail( bam_e2e_harness_t * h ) {
  bam_e2e_batch_def_t ok = bam_e2e_make_batch( 16U, 100UL, 0U, 1U );
  bam_e2e_batch_def_t fail = bam_e2e_make_batch( 17U, 100UL, 0U, 1U );
  fail.txn[0].mode = BAM_E2E_TXN_EXEC_FAIL;
  bam_e2e_node_deliver_batches( h, &ok, 1UL, 0U );
  bam_e2e_node_deliver_batches( h, &fail, 1UL, 0U );
  bam_e2e_apply_pipeline( h );

  FD_TEST( h->sut_result_cnt==2UL );
  FD_TEST( h->sut_results[0].execution_success==1U );
  FD_TEST( h->sut_results[1].execution_success==1U ); /* fee-only commit */
  FD_TEST( h->sut_results[1].transaction_err_count==1U );
}
...
bam_e2e_run_scenario_replay_same_seq( bam_e2e_harness_t * h ) {
  ...
  FD_TEST( h->sut_results[1].execution_success==0U );
}

static void
bam_e2e_run_scenario_replay_new_seq_after_missing_result( bam_e2e_harness_t * h ) {
  bam_e2e_batch_def_t b0 = bam_e2e_make_batch( 19U, 100UL, 1U, 2U );
  bam_e2e_batch_def_t b1 = b0;
  b1.seq_id = 20U;

  bam_e2e_node_deliver_batches( h, &b0, 1UL, 0U );
  bam_e2e_apply_pipeline( h );
  ulong fee_after_first = h->fee_lamports_total;
...
static void
bam_e2e_run_scenario_disconnect_reconnect( bam_e2e_harness_t * h ) {
  bam_e2e_batch_def_t b = bam_e2e_make_batch( 21U, 100UL, 1U, 2U );
  bam_e2e_node_deliver_batches( h, &b, 1UL, 0U );
  bam_e2e_apply_pipeline( h );

  FD_TEST( h->state->bam_pending_results>=1U );
...
static void
bam_e2e_run_scenario_limit_edges( bam_e2e_harness_t * h ) {
  h->slot_cu_limit = 30U;
  h->slot_microblock_limit = 2U;

  bam_e2e_batch_def_t ok = bam_e2e_make_batch( 22U, 100UL, 1U, 1U );
  ok.txn[0].requested_cu = 20U;
  ok.txn[0].actual_cu    = 20U;

  bam_e2e_batch_def_t overflow = bam_e2e_make_batch( 23U, 100UL, 1U, 2U );
  overflow.txn[0].requested_cu = 20U;
  overflow.txn[1].requested_cu = 20U;

  bam_e2e_node_deliver_batches( h, &ok, 1UL, 0U );
  bam_e2e_node_deliver_batches( h, &overflow, 1UL, 0U );
  bam_e2e_apply_pipeline( h );
...
static void
bam_e2e_run_scenario_fee_accounting( bam_e2e_harness_t * h ) {
  bam_e2e_batch_def_t a = bam_e2e_make_batch( 24U, 100UL, 0U, 1U );
  bam_e2e_batch_def_t b = bam_e2e_make_batch( 25U, 100UL, 1U, 2U );
  b.txn[0].mode = BAM_E2E_TXN_EXEC_FAIL;
  bam_e2e_batch_def_t c = bam_e2e_make_batch( 26U, 100UL, 0U, 1U );

  bam_e2e_node_deliver_batches( h, &a, 1UL, 0U );
  bam_e2e_node_deliver_batches( h, &b, 1UL, 0U );
  bam_e2e_node_deliver_batches( h, &c, 1UL, 0U );
  bam_e2e_apply_pipeline( h );
...
static void
bam_e2e_run_scenario_forced_saturation( bam_e2e_harness_t * h ) {
  ...
  h->state->bam_pending_results = FD_BAM_MAX_PENDING_RESULTS;
  bam_e2e_try_enqueue_result( h, &fill );
  FD_TEST( h->state->metrics.bundle_result_drop_cnt > before_drop );
}

static void
bam_e2e_run_scenarios( fd_wksp_t * wksp ) {
  bam_e2e_harness_t h[1];

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_auth_and_control( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_prevalidation_too_many_packets( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_prevalidation_inconsistent_revert( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_prevalidation_stale_slot( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );
  ...
  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_non_leader_phase( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_bank_unavailable_timeout( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_atomic_success( h );
  bam_e2e_assert_trace_metadata( h );
  bam_e2e_assert_intents( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_atomic_verify_fail( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_atomic_lut_fail( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_atomic_exec_fail( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_partial_then_seq_switch( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );
  ...
  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_replay_same_seq( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_replay_new_seq_after_missing_result( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_disconnect_reconnect( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_limit_edges( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_fee_accounting( h );
  bam_e2e_assert_differential( h );
  bam_e2e_fini( h );

  bam_e2e_init( h, wksp );
  bam_e2e_run_scenario_forced_saturation( h );
  bam_e2e_fini( h );
}
