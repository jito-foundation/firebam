...
/* TRAVERSE_LEVEL Takes in and updates s (a vector with all elements set
   ...
   fd_chacha_rng_ulong_roll's contract, we know x<ULONG_MAX, so forming
...
fd_wsample_sample_and_remove_many( fd_wsample_t * sampler,
  ...
  for( ulong i=0UL; i<cnt; i++ ) {
    ...
    ulong unif = fd_chacha_rng_ulong_roll( sampler->rng, sampler->unremoved_weight+sampler->poisoned_weight );
