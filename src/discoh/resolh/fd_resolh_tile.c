...
typedef struct fd_resolh_out fd_resolh_out_t;
...
after_frag( fd_resolh_tile_t *  ctx,
  ...
  if( FD_UNLIKELY( !blockhash && !is_bam && !txnm->block_engine.bundle_id &&
                   !fd_resolh_is_durable_nonce( txnt, fd_txn_m_payload( txnm ) ) ) ) {
    ulong pool_idx;
...
#ifdef FD_RESOLH_TILE_BAM_UNIT_TEST
