...
ulong
fd_guih_footprint( ulong tile_cnt ) {
  FD_TEST( tile_cnt && tile_cnt <=FD_TOPO_MAX_TILES );

  ulong l = FD_LAYOUT_INIT;
  l = FD_LAYOUT_APPEND( l, fd_guih_align(),                sizeof(fd_guih_t) );
  l = FD_LAYOUT_APPEND( l, alignof(fd_guih_tile_timers_t), FD_GUIH_TILE_TIMER_SNAP_CNT * tile_cnt * sizeof(fd_guih_tile_timers_t) );
  l = FD_LAYOUT_APPEND( l, alignof(fd_guih_tile_timers_t), FD_GUIH_LEADER_CNT * FD_GUIH_TILE_TIMER_LEADER_DOWNSAMPLE_CNT * tile_cnt * sizeof(fd_guih_tile_timers_t) );
  l = FD_LAYOUT_APPEND( l, fd_guih_rate_deque_align(),     fd_guih_rate_deque_footprint() ); /* ingress_maxq */
  l = FD_LAYOUT_APPEND( l, fd_guih_rate_deque_align(),     fd_guih_rate_deque_footprint() ); /* egress_maxq  */
  return FD_LAYOUT_FINI( l, fd_guih_align() );
}

void *
fd_guih_new( void *                shmem,
            ...
            int                   is_full_client,
            int                   snapshots_enabled FD_PARAM_UNUSED,
            int                   is_voting,
            int                   schedule_strategy,
            char const *          wfs_expected_bank_hash_cstr FD_PARAM_UNUSED,
  ...
  if( FD_UNLIKELY( !fd_ulong_is_aligned( (ulong)shmem, fd_guih_align() ) ) ) {
  ...
  fd_guih_t *             gui                  = FD_SCRATCH_ALLOC_APPEND( l, fd_guih_align(),                sizeof(fd_guih_t) );
  fd_guih_tile_timers_t * tile_timers_snap_mem = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_guih_tile_timers_t), FD_GUIH_TILE_TIMER_SNAP_CNT * tile_cnt * sizeof(fd_guih_tile_timers_t) );
  fd_guih_tile_timers_t * leader_tt_mem        = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_guih_tile_timers_t), FD_GUIH_LEADER_CNT * FD_GUIH_TILE_TIMER_LEADER_DOWNSAMPLE_CNT * tile_cnt * sizeof(fd_guih_tile_timers_t) );
  void *                 ingress_maxq_mem     = FD_SCRATCH_ALLOC_APPEND( l, fd_guih_rate_deque_align(),     fd_guih_rate_deque_footprint() );
  ...
  gui->summary.ingress_maxq     = fd_guih_rate_deque_join( fd_guih_rate_deque_new( ingress_maxq_mem ) );
  gui->summary.egress_maxq      = fd_guih_rate_deque_join( fd_guih_rate_deque_new( egress_maxq_mem  ) );
  ...
  for( ulong i=0UL; i<FD_GUIH_LEADER_CNT; i++ ) gui->leader_slots[ i ]->tile_timers = leader_tt_mem + i * FD_GUIH_TILE_TIMER_LEADER_DOWNSAMPLE_CNT * tile_cnt;
  ...
  gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_INITIALIZING;
  ...
  gui->summary.vote_state = is_voting ? FD_GUIH_VOTE_STATE_VOTING : FD_GUIH_VOTE_STATE_NON_VOTING;
  ...
  for( ulong i=0UL; i < (FD_GUIH_REPAIR_SLOT_HISTORY_SZ+1UL); i++ )  gui->summary.slots_max_repair[ i ].slot  = ULONG_MAX;
  for( ulong i=0UL; i < (FD_GUIH_TURBINE_SLOT_HISTORY_SZ+1UL); i++ ) gui->summary.slots_max_turbine[ i ].slot = ULONG_MAX;

  for( ulong i=0UL; i < FD_GUIH_TURBINE_RECV_TIMESTAMPS; i++ ) gui->turbine_slots[ i ].slot = ULONG_MAX;
  ...
  memset( gui->summary.tile_timers_snap,            0, tile_cnt * sizeof(fd_guih_tile_timers_t) );
  memset( gui->summary.tile_timers_snap + tile_cnt, 0, tile_cnt * sizeof(fd_guih_tile_timers_t) );
  ...
  for( ulong i=0UL; i<FD_GUIH_SLOTS_CNT;  i++ ) gui->slots[ i ]->slot             = ULONG_MAX;
  for( ulong i=0UL; i<FD_GUIH_LEADER_CNT; i++ ) gui->leader_slots[ i ]->slot      = ULONG_MAX;
...
fd_guih_t *
fd_guih_join( void * shmem ) {
  return (fd_guih_t *)shmem;
}

void
fd_guih_set_identity( fd_guih_t *    gui,
  ...
  fd_guih_printf_identity_key( gui );
  fd_http_server_ws_broadcast( gui->http );
}

void
fd_guih_ws_open( fd_guih_t * gui,
                ulong      ws_conn_id,
                long now ) {
  void (* printers[] )( fd_guih_t * gui ) = {
    fd_guih_printf_startup_progress,
    fd_guih_printf_version,
    fd_guih_printf_cluster,
    fd_guih_printf_commit_hash,
    fd_guih_printf_identity_key,
    fd_guih_printf_vote_key,
    fd_guih_printf_startup_time_nanos,
    fd_guih_printf_vote_state,
    fd_guih_printf_vote_distance,
    fd_guih_printf_turbine_slot,
    fd_guih_printf_repair_slot,
    fd_guih_printf_slot_caught_up,
    fd_guih_printf_tps_history,
    fd_guih_printf_tiles,
    fd_guih_printf_schedule_strategy,
    fd_guih_printf_identity_balance,
    fd_guih_printf_vote_balance,
    fd_guih_printf_estimated_slot_duration_nanos,
    fd_guih_printf_root_slot,
    fd_guih_printf_storage_slot,
    fd_guih_printf_reset_slot,
    fd_guih_printf_active_fork_cnt,
    fd_guih_printf_optimistically_confirmed_slot,
    fd_guih_printf_completed_slot,
    fd_guih_printf_estimated_slot,
    fd_guih_printf_live_tile_timers,
    fd_guih_printf_live_tile_metrics,
    fd_guih_printf_catch_up_history,
    fd_guih_printf_vote_latency_history,
    fd_guih_printf_late_votes_history,
    fd_guih_printf_health
  };
  ...
  for( ulong i=0UL; i<2UL; i++ ) {
    if( FD_LIKELY( gui->epoch.has_epoch[ i ] ) ) {
      fd_guih_printf_skip_rate( gui, i );
      FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
      fd_guih_printf_epoch( gui, i );
      FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
    }
  }

  ulong epoch_idx = fd_guih_current_epoch_idx( gui );
  if( FD_LIKELY( epoch_idx!=ULONG_MAX ) ) {
    fd_guih_printf_skipped_history( gui, epoch_idx );
    FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
    fd_guih_printf_skipped_history_cluster( gui, epoch_idx );
    FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
  }

  /* Print peers last because it's the largest message and would
     block other information. */
  fd_guih_printf_peers_all( gui );
  FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );

  /* rebroadcast 10s of historical shred data */
  if( FD_LIKELY( gui->shreds.staged_next_broadcast!=ULONG_MAX ) ) {
    fd_guih_printf_shred_rebroadcast( gui, now-(long)(10*1e9) );
    FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
  }
}

static void
fd_guih_tile_timers_snap( fd_guih_t * gui ) {
  fd_guih_tile_timers_t * cur = gui->summary.tile_timers_snap + gui->summary.tile_timers_snap_idx * gui->tile_cnt;
  gui->summary.tile_timers_snap_idx = (gui->summary.tile_timers_snap_idx+1UL)%FD_GUIH_TILE_TIMER_SNAP_CNT;
...
static void
fd_guih_scheduler_counts_snap( fd_guih_t * gui, long now ) {
  ulong pack_tile_idx = fd_topo_find_tile( gui->topo, "pack", 0UL );
  if( FD_UNLIKELY( pack_tile_idx==ULONG_MAX ) ) return;

  fd_guih_scheduler_counts_t * cur = gui->summary.scheduler_counts_snap[ gui->summary.scheduler_counts_snap_idx ];
  gui->summary.scheduler_counts_snap_idx = (gui->summary.scheduler_counts_snap_idx+1UL)%FD_GUIH_SCHEDULER_COUNT_SNAP_CNT;
...
static void
fd_guih_estimated_tps_snap( fd_guih_t * gui ) {
  ...
  for( ulong i=0UL; i<fd_ulong_min( gui->summary.slot_completed+1UL, FD_GUIH_SLOTS_CNT ); i++ ) {
    ulong _slot = gui->summary.slot_completed-i;
    fd_guih_slot_t const * slot = fd_guih_get_slot_const( gui, _slot );
    ...
    if( FD_UNLIKELY( slot->completed_time+FD_GUIH_TPS_HISTORY_WINDOW_DURATION_SECONDS*1000L*1000L*1000L<gui->next_sample_400millis ) ) break; /* Slot too old. */
  ...
  gui->summary.estimated_tps_history_idx = (gui->summary.estimated_tps_history_idx+1UL) % FD_GUIH_TPS_HISTORY_SAMPLE_CNT;
}

static void
fd_guih_network_stats_snap_egress( fd_topo_t const *         topo,
                                   char const *              tile_name,
                                   fd_guih_network_stats_t * cur ) {
...
static void
fd_guih_network_stats_snap( fd_guih_t *               gui,
                           fd_guih_network_stats_t * cur ) {
  ...
  cur->in.gossip   = fd_guih_metrics_gossip_total_ingress_bytes( topo, gossvf_tile_cnt );
  cur->out.gossip  = fd_guih_metrics_gossip_total_egress_bytes( topo, gossip_tile_cnt );
  cur->in.turbine  = fd_guih_metrics_sum_tiles_counter( topo, "shred", shred_tile_cnt, MIDX( COUNTER, SHRED, SHRED_TURBINE_RX_BYTES ) );

  cur->out.turbine = 0UL;
  cur->out.repair  = 0UL;
  cur->out.rserve  = 0UL; /* rserve is not part of frankendancer */
  cur->out.tpu     = 0UL;
  fd_guih_network_stats_snap_egress( topo, "net",  cur );
  fd_guih_network_stats_snap_egress( topo, "mlx5", cur );

  cur->in.repair = fd_guih_metrics_sum_tiles_counter( topo, "shred", shred_tile_cnt, MIDX( COUNTER, SHRED, SHRED_REPAIR_RX_BYTES ) );
  ...
  cur->in.rserve = 0UL; /* rserve is not part of frankendancer */
...
static void
fd_guih_network_rate_max_update( fd_guih_t * gui,
                                long       now ) {
  fd_guih_network_stats_t * cur  = gui->summary.network_stats_current;
  fd_guih_network_stats_t * prev = gui->summary.network_stats_prev;
  ...
  ulong d_in[ FD_GUIH_NET_PROTO_CNT ];
  ...
  ulong d_out[ FD_GUIH_NET_PROTO_CNT ];
  ...
  if( FD_LIKELY( dt_ns>0L ) ) {
    ...
    for( ulong i=0UL; i<FD_GUIH_NET_PROTO_CNT; i++ ) {
      ...
      if( FD_UNLIKELY( !gui->summary.net_rate_ema_ready ) ) {
        ...
        gui->summary.ingress_ema[ i ] = fd_guih_ema( gui->summary.net_rate_prev_ts, now, rate_in,  gui->summary.ingress_ema[ i ], FD_GUIH_NETWORK_EMA_HALF_LIFE_NS );
        gui->summary.egress_ema[ i ]  = fd_guih_ema( gui->summary.net_rate_prev_ts, now, rate_out, gui->summary.egress_ema[ i ],  FD_GUIH_NETWORK_EMA_HALF_LIFE_NS );
      }
    }
  ...
  if( FD_LIKELY( gui->summary.net_rate_ema_ready ) ) {
    ...
    while( !fd_guih_rate_deque_empty( gui->summary.ingress_maxq ) && fd_guih_rate_deque_peek_head_const( gui->summary.ingress_maxq )->ts_nanos<now-FD_GUIH_NET_RATE_MAX_WINDOW_NS ) {
      fd_guih_rate_deque_pop_head( gui->summary.ingress_maxq );
    }
    while( !fd_guih_rate_deque_empty( gui->summary.ingress_maxq ) && fd_guih_rate_deque_peek_tail_const( gui->summary.ingress_maxq )->value<=sum_in ) {
      fd_guih_rate_deque_pop_tail( gui->summary.ingress_maxq );
    }
    if( FD_UNLIKELY( fd_guih_rate_deque_full( gui->summary.ingress_maxq ) ) ) {
      fd_guih_rate_deque_pop_tail( gui->summary.ingress_maxq );
    }
    fd_guih_rate_deque_push_tail( gui->summary.ingress_maxq, (fd_guih_rate_entry_t){ .ts_nanos=now, .value=sum_in } );

    while( !fd_guih_rate_deque_empty( gui->summary.egress_maxq ) && fd_guih_rate_deque_peek_head_const( gui->summary.egress_maxq )->ts_nanos<now-FD_GUIH_NET_RATE_MAX_WINDOW_NS ) {
      fd_guih_rate_deque_pop_head( gui->summary.egress_maxq );
    }
    while( !fd_guih_rate_deque_empty( gui->summary.egress_maxq ) && fd_guih_rate_deque_peek_tail_const( gui->summary.egress_maxq )->value<=sum_out ) {
      fd_guih_rate_deque_pop_tail( gui->summary.egress_maxq );
    }
    if( FD_UNLIKELY( fd_guih_rate_deque_full( gui->summary.egress_maxq ) ) ) {
      fd_guih_rate_deque_pop_tail( gui->summary.egress_maxq );
    }
    fd_guih_rate_deque_push_tail( gui->summary.egress_maxq, (fd_guih_rate_entry_t){ .ts_nanos=now, .value=sum_out } );
  }
...
static void
fd_guih_txn_waterfall_snap( fd_guih_t *               gui,
                           fd_guih_txn_waterfall_t * cur ) {
  memset( cur, 0, sizeof(fd_guih_txn_waterfall_t) );
  fd_topo_t const * topo = gui->topo;
  ...
  if( dedup_tile_idx!=ULONG_MAX ) {
    fd_topo_tile_t const * dedup = &topo->tiles[ dedup_tile_idx ];
    volatile ulong const * dedup_metrics = fd_metrics_tile( dedup->metrics );
    cur->in.gossip = dedup_metrics[ MIDX( COUNTER, DEDUP, VOTE_GOSSIP_RX ) ];
  }
...
static void
fd_guih_tile_stats_snap( fd_guih_t *                     gui,
                        fd_guih_txn_waterfall_t const * waterfall,
                        fd_guih_tile_stats_t *          stats,
                        long                           now ) {
  memset( stats, 0, sizeof(fd_guih_tile_stats_t) );
  fd_topo_t const * topo = gui->topo;
...
static inline int
fd_guih_ephemeral_slots_contains( fd_guih_ephemeral_slot_t * slots, ulong slots_sz, ulong slot ) {
...
#define SORT_NAME fd_guih_ephemeral_slot_sort
#define SORT_KEY_T fd_guih_ephemeral_slot_t
...
static inline void
fd_guih_try_insert_ephemeral_slot( fd_guih_ephemeral_slot_t * slots, ulong slots_sz, ulong slot, long now ) {
  ...
  fd_guih_ephemeral_slot_sort_insert( slots, slots_sz+1UL );
}

static inline void
fd_guih_try_insert_run_length_slot( ulong * slots, ulong capacity, ulong * slots_sz, ulong slot ) {
...
void
fd_guih_handle_repair_slot( fd_guih_t * gui, ulong slot, long now ) {
  int was_sent = fd_guih_ephemeral_slots_contains( gui->summary.slots_max_repair, FD_GUIH_REPAIR_SLOT_HISTORY_SZ, slot );
  fd_guih_try_insert_ephemeral_slot( gui->summary.slots_max_repair, FD_GUIH_REPAIR_SLOT_HISTORY_SZ, slot, now );

  if( FD_UNLIKELY( !was_sent && slot!=gui->summary.slot_repair ) ) {
    gui->summary.slot_repair = slot;

    fd_guih_printf_repair_slot( gui );
    fd_http_server_ws_broadcast( gui->http );

    if( FD_UNLIKELY( gui->summary.slot_caught_up==ULONG_MAX ) ) fd_guih_try_insert_run_length_slot( gui->summary.catch_up_repair, FD_GUIH_REPAIR_CATCH_UP_HISTORY_SZ, &gui->summary.catch_up_repair_sz, slot );
  }
}

int
fd_guih_poll( fd_guih_t * gui, long now ) {
  if( FD_LIKELY( now>gui->next_sample_400millis ) ) {
    fd_guih_estimated_tps_snap( gui );
    fd_guih_printf_estimated_tps( gui );
    fd_http_server_ws_broadcast( gui->http );
  ...
  if( FD_LIKELY( now>gui->next_sample_100millis ) ) {
    fd_guih_txn_waterfall_snap( gui, gui->summary.txn_waterfall_current );
    fd_guih_printf_live_txn_waterfall( gui, gui->summary.txn_waterfall_reference, gui->summary.txn_waterfall_current, 0UL /* TODO: REAL NEXT LEADER SLOT */ );
    fd_http_server_ws_broadcast( gui->http );

    fd_guih_network_stats_snap( gui, gui->summary.network_stats_current );
    fd_guih_network_rate_max_update( gui, now );
    fd_guih_printf_live_network_metrics( gui, gui->summary.network_stats_current );
    fd_http_server_ws_broadcast( gui->http );

    *gui->summary.tile_stats_reference = *gui->summary.tile_stats_current;
    fd_guih_tile_stats_snap( gui, gui->summary.txn_waterfall_current, gui->summary.tile_stats_current, now );
    fd_guih_printf_live_tile_stats( gui, gui->summary.tile_stats_reference, gui->summary.tile_stats_current );
    fd_http_server_ws_broadcast( gui->http );
  ...
  if( FD_LIKELY( now>gui->next_sample_50millis ) ) {
    ...
    if( FD_LIKELY( gui->summary.slot_caught_up!=ULONG_MAX ) ) {
      ...
      fd_guih_handle_repair_slot( gui, slot, now );
    }
  ...
  if( FD_LIKELY( now>gui->next_sample_25millis ) ) {
    ...
    fd_guih_printf_live_tile_timers( gui );
    fd_http_server_ws_broadcast( gui->http );

    fd_guih_printf_live_tile_metrics( gui );
    fd_http_server_ws_broadcast( gui->http );
  ...
  if( FD_LIKELY( now>gui->next_sample_10millis ) ) {
    ...
    fd_guih_printf_server_time_nanos( gui, now );
    fd_http_server_ws_broadcast( gui->http );
...
fd_guih_handle_gossip_update( fd_guih_t *   gui,
  ...
  ulong added_cnt = 0UL;
  ulong added[ FD_GUIH_MAX_PEER_CNT ] = {0};

  ulong update_cnt = 0UL;
  ulong updated[ FD_GUIH_MAX_PEER_CNT ] = {0};
  ...
  fd_guih_printf_peers_gossip_update( gui, updated, update_cnt, removed, removed_cnt, added, added_cnt );
  fd_http_server_ws_broadcast( gui->http );
}
...
fd_guih_handle_vote_account_update( fd_guih_t *   gui,
  ...
  fd_guih_printf_peers_vote_account_update( gui, updated, update_cnt, removed, removed_cnt, added, added_cnt );
  fd_http_server_ws_broadcast( gui->http );
}
...
fd_guih_handle_validator_info_update( fd_guih_t *   gui,
  ...
  fd_guih_printf_peers_validator_info_update( gui, updated, update_cnt, NULL, removed_cnt, added, added_cnt );
  fd_http_server_ws_broadcast( gui->http );
}

int
fd_guih_request_slot( fd_guih_t *    gui,
                     ulong         ws_conn_id,
                     ulong         request_id,
                     cJSON const * params ) {
  ...
  fd_guih_slot_t const * slot = fd_guih_get_slot_const( gui, _slot );
  if( FD_UNLIKELY( !slot ) ) {
    fd_guih_printf_null_query_response( gui->http, "slot", "query", request_id );
    FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
    return 0;
  }

  fd_guih_printf_slot_request( gui, _slot, request_id );
  FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
  return 0;
}

int
fd_guih_request_slot_transactions( fd_guih_t *    gui,
                                  ulong         ws_conn_id,
                                  ulong         request_id,
                                  cJSON const * params ) {
  const cJSON * slot_param = cJSON_GetObjectItemCaseSensitive( params, "slot" );
  ...
  fd_guih_slot_t const * slot = fd_guih_get_slot_const( gui, _slot );
  if( FD_UNLIKELY( !slot ) ) {
    fd_guih_printf_null_query_response( gui->http, "slot", "query_transactions", request_id );
    FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
    return 0;
  }

  fd_guih_printf_slot_transactions_request( gui, _slot, request_id );
  FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
  return 0;
}

int
fd_guih_request_slot_detailed( fd_guih_t *    gui,
                              ulong         ws_conn_id,
                              ulong         request_id,
                              cJSON const * params ) {
  const cJSON * slot_param = cJSON_GetObjectItemCaseSensitive( params, "slot" );
  ...
  fd_guih_slot_t const * slot = fd_guih_get_slot_const( gui, _slot );
  if( FD_UNLIKELY( !slot ) ) {
    fd_guih_printf_null_query_response( gui->http, "slot", "query_detailed", request_id );
    FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
    return 0;
  }

  fd_guih_printf_slot_request_detailed( gui, _slot, request_id );
  FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
  return 0;
}

static inline ulong
fd_guih_slot_duration( fd_guih_t const * gui, fd_guih_slot_t const * cur ) {
  fd_guih_slot_t const * prev = fd_guih_get_slot_const( gui, cur->slot-1UL );
  if( FD_UNLIKELY( !prev ||
                   prev->skipped ||
...
/* All rankings are initialized / reset to ULONG_MAX.  These sentinels
   ...
   type. */
#define SORT_NAME fd_guih_slot_ranking_sort
#define SORT_KEY_T fd_guih_slot_ranking_t
#define SORT_BEFORE(a,b) fd_int_if( (a).slot==ULONG_MAX, 0, fd_int_if( (b).slot==ULONG_MAX, 1, fd_int_if( (a).value==(b).value, (a).slot>(b).slot, fd_int_if( (a).type==FD_GUIH_SLOT_RANKING_TYPE_DESC, (a).value>(b).value, (a).value<(b).value ) ) ) )
#include "../../util/tmpl/fd_sort.c"

static inline void
fd_guih_try_insert_ranking( fd_guih_t               * gui,
                           fd_guih_slot_rankings_t * rankings,
                           fd_guih_slot_t const    * slot ) {
...
#define TRY_INSERT_SLOT( ranking_name, ranking_slot, ranking_value ) \
  do { \
    rankings->FD_CONCAT2(largest_, ranking_name) [ FD_GUIH_SLOT_RANKINGS_SZ ] = (fd_guih_slot_ranking_t){ .slot = (ranking_slot), .value = (ranking_value), .type = FD_GUIH_SLOT_RANKING_TYPE_DESC }; \
    fd_guih_slot_ranking_sort_insert( rankings->FD_CONCAT2(largest_, ranking_name), FD_GUIH_SLOT_RANKINGS_SZ+1UL ); \
    rankings->FD_CONCAT2(smallest_, ranking_name)[ FD_GUIH_SLOT_RANKINGS_SZ ] = (fd_guih_slot_ranking_t){ .slot = (ranking_slot), .value = (ranking_value), .type = FD_GUIH_SLOT_RANKING_TYPE_ASC  }; \
    fd_guih_slot_ranking_sort_insert( rankings->FD_CONCAT2(smallest_, ranking_name), FD_GUIH_SLOT_RANKINGS_SZ+1UL ); \
  } while (0)
    ...
    ulong dur = fd_guih_slot_duration( gui, slot );
...
static void
fd_guih_update_slot_rankings( fd_guih_t * gui ) {
  ...
  for( ulong s = gui->summary.slot_rooted; s>=gui->epoch.epochs[ epoch_idx ].rankings_slot; s--) {
    fd_guih_slot_t const * slot = fd_guih_get_slot_const( gui, s );
    if( FD_UNLIKELY( !slot ) ) break;

    fd_guih_try_insert_ranking( gui, gui->epoch.epochs[ epoch_idx ].rankings, slot );
    if( FD_UNLIKELY( slot->mine ) ) fd_guih_try_insert_ranking( gui, gui->epoch.epochs[ epoch_idx ].my_rankings, slot );
  }
...
int
fd_guih_request_slot_rankings( fd_guih_t *    gui,
                              ulong         ws_conn_id,
                              ulong         request_id,
                              cJSON const * params ) {
  ...
  fd_guih_update_slot_rankings( gui );
  fd_guih_printf_slot_rankings_request( gui, request_id, mine );
  FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
  return 0;
}

int
fd_guih_request_slot_shreds( fd_guih_t *    gui,
                            ulong         ws_conn_id,
                            ulong         request_id,
                            cJSON const * params ) {
  const cJSON * slot_param = cJSON_GetObjectItemCaseSensitive( params, "slot" );
  ...
  fd_guih_slot_t const * slot = fd_guih_get_slot_const( gui, _slot );
  if( FD_UNLIKELY( !slot || slot->shreds.start_offset==ULONG_MAX || slot->shreds.end_offset==ULONG_MAX || gui->shreds.history_tail >= slot->shreds.end_offset + FD_GUIH_SHREDS_HISTORY_SZ ) ) {
    fd_guih_printf_null_query_response( gui->http, "slot", "query_shreds", request_id );
    FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
    return 0;
  }

  fd_guih_printf_slot_query_shreds( gui, _slot, request_id );
  FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
  return 0;
}

int
fd_guih_ws_message( fd_guih_t *    gui,
                   ulong         ws_conn_id,
                   uchar const * data,
                   ulong         data_len ) {
  ...
  if( FD_LIKELY( !strcmp( topic->valuestring, "slot" ) && !strcmp( key->valuestring, "query" ) ) ) {
    ...
    int result = fd_guih_request_slot( gui, ws_conn_id, id, params );
    cJSON_Delete( json );
    return result;
    ...
    int result = fd_guih_request_slot_detailed( gui, ws_conn_id, id, params );
    cJSON_Delete( json );
    return result;
    ...
    int result = fd_guih_request_slot_transactions( gui, ws_conn_id, id, params );
    cJSON_Delete( json );
    return result;
    ...
    int result = fd_guih_request_slot_rankings( gui, ws_conn_id, id, params );
    cJSON_Delete( json );
    return result;
    ...
    int result = fd_guih_request_slot_shreds( gui, ws_conn_id, id, params );
    cJSON_Delete( json );
    return result;
  } else if( FD_LIKELY( !strcmp( topic->valuestring, "summary" ) && !strcmp( key->valuestring, "ping" ) ) ) {
    fd_guih_printf_summary_ping( gui, id );
    FD_TEST( !fd_http_server_ws_send( gui->http, ws_conn_id ) );
...
static fd_guih_slot_t *
fd_guih_clear_slot( fd_guih_t *      gui,
                   ulong           _slot,
                   ulong           _parent_slot ) {
  fd_guih_slot_t * slot = gui->slots[ _slot % FD_GUIH_SLOTS_CNT ];
  ...
  slot->level                  = FD_GUIH_SLOT_LEVEL_INCOMPLETE;
  ...
  if( FD_LIKELY( slot->mine ) ) {
    ...
    fd_guih_leader_slot_t * lslot = gui->leader_slots[ slot->leader_history_idx % FD_GUIH_LEADER_CNT ];
  ...
  if( FD_UNLIKELY( !_slot ) ) {
    ...
    slot->level   = FD_GUIH_SLOT_LEVEL_ROOTED;
  }
...
fd_guih_handle_leader_schedule( fd_guih_t *                    gui,
  ...
  if( FD_UNLIKELY( leader_schedule->start_slot==0UL ) ) {
    ...
    for( ulong i=0UL; i<fd_ulong_min( leader_schedule->start_slot-1UL, FD_GUIH_SLOTS_CNT ); i++ ) {
      fd_guih_slot_t const * slot = fd_guih_get_slot_const( gui, leader_schedule->start_slot-i );
      if( FD_UNLIKELY( !slot ) ) break;
...
#define DOWNSAMPLE( a, a_start, a_end, a_capacity, b, b_sz, stride ) (__extension__({  \
  ...
  __cnt; }))
    ...
    lslot->tile_timers_sample_cnt = DOWNSAMPLE(
      ...
      FD_GUIH_TILE_TIMER_SNAP_CNT,
      lslot->tile_timers,
      FD_GUIH_TILE_TIMER_LEADER_DOWNSAMPLE_CNT,
      gui->tile_cnt );
    ...
    lslot->scheduler_counts_sample_cnt = DOWNSAMPLE(
      ...
      FD_GUIH_SCHEDULER_COUNT_SNAP_CNT,
      lslot->scheduler_counts,
      FD_GUIH_SCHEDULER_COUNT_LEADER_DOWNSAMPLE_CNT,
      1UL );
#undef DOWNSAMPLE
  }
  ...
  fd_guih_txn_waterfall_snap( gui, slot->waterfall_end );
...
fd_guih_handle_reset_slot_legacy( fd_guih_t *    gui,
  ...
  if( FD_LIKELY( gui->summary.vote_state!=FD_GUIH_VOTE_STATE_NON_VOTING ) ) {
    if( FD_UNLIKELY( last_landed_vote==ULONG_MAX || (last_landed_vote+150UL)<_slot ) ) {
      ...
      if( FD_UNLIKELY( gui->summary.vote_state!=FD_GUIH_VOTE_STATE_VOTING ) ) {
        gui->summary.vote_state = FD_GUIH_VOTE_STATE_VOTING;
        fd_guih_printf_vote_state( gui );
        fd_http_server_ws_broadcast( gui->http );
      }
    }
  ...
  for( ulong i=0UL; i<fd_ulong_min( _slot+1, FD_GUIH_SLOTS_CNT ); i++ ) {
    ulong parent_slot = _slot - i;

    fd_guih_slot_t * slot = fd_guih_get_slot( gui, parent_slot );
    if( FD_UNLIKELY( !slot ) ) slot = fd_guih_clear_slot( gui, parent_slot, ULONG_MAX );
    ...
    if( FD_LIKELY( slot->level>=FD_GUIH_SLOT_LEVEL_ROOTED ) ) break;
    ...
    if( FD_LIKELY( should_republish ) ) {
      fd_guih_printf_slot( gui, parent_slot );
      fd_http_server_ws_broadcast( gui->http );
    }
  ...
  for( ulong i=0UL; i<fd_ulong_min( _slot+1, slot_duration_history_sz ); i++ ) {
    ...
    fd_guih_slot_t const * slot = fd_guih_get_slot_const( gui, parent_slot );
    if( FD_UNLIKELY( !slot) ) break;
    ...
    ulong slot_duration = fd_guih_slot_duration( gui, slot );
  ...
  if( FD_LIKELY( gui->summary.slot_completed==ULONG_MAX || _slot!=gui->summary.slot_completed ) ) {
    ...
    if( FD_UNLIKELY( gui->summary.slots_max_turbine[ 0 ].slot!=ULONG_MAX && gui->summary.slot_completed!=ULONG_MAX && gui->summary.slot_completed>gui->summary.slots_max_turbine[ 0 ].slot ) ) {
      fd_guih_try_insert_ephemeral_slot( gui->summary.slots_max_turbine, FD_GUIH_TURBINE_SLOT_HISTORY_SZ, gui->summary.slot_completed, now );
    }

    int slot_turbine_hist_full = gui->summary.slots_max_turbine[ FD_GUIH_TURBINE_SLOT_HISTORY_SZ-1UL ].slot!=ULONG_MAX;
    if( FD_UNLIKELY( gui->summary.slot_caught_up==ULONG_MAX && slot_turbine_hist_full && gui->summary.slots_max_turbine[ 0 ].slot < (gui->summary.slot_completed + 3UL) ) ) {
      ...
      fd_guih_printf_slot_caught_up( gui );
      fd_http_server_ws_broadcast( gui->http );
    }
  }
...
static void
fd_guih_handle_completed_slot( fd_guih_t *    gui,
                              uchar const * msg,
                              long          now ) {
  ...
  fd_guih_slot_t * slot = fd_guih_get_slot( gui, _slot );
  if( FD_UNLIKELY( !slot ) ) slot = fd_guih_clear_slot( gui, _slot, _parent_slot );

  slot->completed_time = now;
  slot->parent_slot = _parent_slot;
  slot->max_compute_units = (uint)max_compute_units;
  if( FD_LIKELY( slot->level<FD_GUIH_SLOT_LEVEL_COMPLETED ) ) {
    ...
    if( FD_LIKELY( gui->summary.slot_optimistically_confirmed!=ULONG_MAX && _slot<gui->summary.slot_optimistically_confirmed ) ) {
      ...
      slot->level = FD_GUIH_SLOT_LEVEL_OPTIMISTICALLY_CONFIRMED;
    } else {
      slot->level = FD_GUIH_SLOT_LEVEL_COMPLETED;
    }
  }
  ...
  slot->compute_units          = fd_uint_if( slot->mine, slot->compute_units, compute_units );
...
static void
fd_guih_handle_rooted_slot_legacy( fd_guih_t *    gui,
                                  uchar const * msg ) {
  ulong _slot = FD_LOAD( ulong, msg );
  ...
  /* Slot 0 is always rooted.  No need to iterate all the way back to
     i==_slot */
  for( ulong i=0UL; i<fd_ulong_min( _slot, FD_GUIH_SLOTS_CNT ); i++ ) {
    ulong parent_slot = _slot - i;
    ...
    if( FD_UNLIKELY( slot->level>=FD_GUIH_SLOT_LEVEL_ROOTED ) ) break;
...
static void
fd_guih_handle_optimistically_confirmed_slot( fd_guih_t * gui,
                                             ulong      _slot ) {
  /* Slot 0 is always rooted.  No need to iterate all the way back to
     i==_slot */
  for( ulong i=0UL; i<fd_ulong_min( _slot, FD_GUIH_SLOTS_CNT ); i++ ) {
    ...
    if( FD_LIKELY( slot->level<FD_GUIH_SLOT_LEVEL_OPTIMISTICALLY_CONFIRMED ) ) {
      slot->level = FD_GUIH_SLOT_LEVEL_OPTIMISTICALLY_CONFIRMED;
      fd_guih_printf_slot( gui, parent_slot );
      fd_http_server_ws_broadcast( gui->http );
    }
  ...
  if( FD_UNLIKELY( gui->summary.slot_optimistically_confirmed!=ULONG_MAX && _slot<gui->summary.slot_optimistically_confirmed ) ) {
    ...
    for( long i_=(long)gui->summary.slot_optimistically_confirmed; i_>=(long)_slot; i_-- ) {
      ulong i = (ulong)i_;
      fd_guih_slot_t * slot = fd_guih_get_slot( gui, i );
      if( FD_UNLIKELY( !slot ) ) break;
  ...
  fd_guih_printf_optimistically_confirmed_slot( gui );
  fd_http_server_ws_broadcast( gui->http );
}

static void
fd_guih_handle_balance_update( fd_guih_t *    gui,
                              uchar const * msg ) {
  switch( FD_LOAD( ulong, msg ) ) {
    case 0UL:
      gui->summary.identity_account_balance = FD_LOAD( ulong, msg + 8UL );
      fd_guih_printf_identity_balance( gui );
      fd_http_server_ws_broadcast( gui->http );
      break;
    case 1UL:
      gui->summary.vote_account_balance = FD_LOAD( ulong, msg + 8UL );
      fd_guih_printf_vote_balance( gui );
      fd_http_server_ws_broadcast( gui->http );
      break;
    default:
...
static void
fd_guih_handle_start_progress( fd_guih_t *    gui,
                              uchar const * msg ) {
  uchar type = msg[ 0 ];
  ...
  switch (type) {
    ...
    case 1: {
      ...
      if( FD_UNLIKELY( gui->summary.startup_progress.startup_got_full_snapshot ) ) {
        gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_SEARCHING_FOR_INCREMENTAL_SNAPSHOT;
        snapshot_type = "incremental";
      } else {
        gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_SEARCHING_FOR_FULL_SNAPSHOT;
        snapshot_type = "full";
      }
    ...
    case 2: {
      ...
      if( FD_LIKELY( is_full_snapshot ) ) {
          gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_DOWNLOADING_FULL_SNAPSHOT;
          ...
          gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_DOWNLOADING_INCREMENTAL_SNAPSHOT;
    ...
    case 4:
      gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_CLEANING_BLOCK_STORE;
      FD_LOG_INFO(( "progress: cleaning block store" ));
      break;
    case 5:
      gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_CLEANING_ACCOUNTS;
      FD_LOG_INFO(( "progress: cleaning accounts" ));
      break;
    case 6:
      gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_LOADING_LEDGER;
      FD_LOG_INFO(( "progress: loading ledger" ));
      break;
    case 7: {
      gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_PROCESSING_LEDGER;
    ...
    case 8:
      gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_STARTING_SERVICES;
      FD_LOG_INFO(( "progress: starting services" ));
      break;
    case 9:
      gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_HALTED;
      FD_LOG_INFO(( "progress: halted" ));
      break;
    case 10: {
      gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_WAITING_FOR_SUPERMAJORITY;
    ...
    case 11:
      gui->summary.startup_progress.phase = FD_GUIH_START_PROGRESS_TYPE_RUNNING;
  ...
  fd_guih_printf_startup_progress( gui );
  fd_http_server_ws_broadcast( gui->http );
}

void
fd_guih_handle_genesis_hash( fd_guih_t *        gui,
                            fd_hash_t const * msg ) {
  ...
  if( FD_LIKELY( strcmp( gui->summary.cluster, cluster_name ) ) ) {
    ...
    fd_guih_printf_cluster( gui );
    fd_http_server_ws_broadcast( gui->http );
  }
}

void
fd_guih_handle_block_engine_update( fd_guih_t *                              gui,
...
fd_guih_plugin_message( fd_guih_t *   gui,
  ...
  switch( plugin_msg ) {
    case FD_PLUGIN_MSG_SLOT_ROOTED:
      fd_guih_handle_rooted_slot_legacy( gui, msg );
      break;
    case FD_PLUGIN_MSG_SLOT_OPTIMISTICALLY_CONFIRMED:
      fd_guih_handle_optimistically_confirmed_slot( gui, FD_LOAD( ulong, msg ) );
      break;
    case FD_PLUGIN_MSG_SLOT_COMPLETED: {
      fd_guih_handle_completed_slot( gui, msg, now );
      break;
    }
    ...
    case FD_PLUGIN_MSG_SLOT_START: {
      ...
      fd_guih_handle_slot_start( gui, slot, parent_slot, now );
      break;
    }
    case FD_PLUGIN_MSG_SLOT_END: {
      ulong slot = FD_LOAD( ulong, msg );
      ulong cus_used = FD_LOAD( ulong, (uchar const *)msg + 8UL );
      fd_guih_handle_slot_end( gui, slot, cus_used, now );
      break;
    }
    case FD_PLUGIN_MSG_GOSSIP_UPDATE: {
      fd_guih_handle_gossip_update( gui, msg );
      break;
    }
    case FD_PLUGIN_MSG_VOTE_ACCOUNT_UPDATE: {
      fd_guih_handle_vote_account_update( gui, msg );
      break;
    }
    case FD_PLUGIN_MSG_VALIDATOR_INFO: {
      fd_guih_handle_validator_info_update( gui, msg );
      break;
    }
    case FD_PLUGIN_MSG_SLOT_RESET: {
      fd_guih_handle_reset_slot_legacy( gui, msg, now );
      break;
    }
    case FD_PLUGIN_MSG_BALANCE: {
      fd_guih_handle_balance_update( gui, msg );
      break;
    }
    case FD_PLUGIN_MSG_START_PROGRESS: {
      fd_guih_handle_start_progress( gui, msg );
      break;
    }
...
void
fd_guih_became_leader( fd_guih_t * gui,
                      ulong      _slot,
  ...
  fd_guih_slot_t * slot = fd_guih_get_slot( gui, _slot );
  if( FD_UNLIKELY( !slot ) ) slot = fd_guih_clear_slot( gui, _slot, ULONG_MAX );
  fd_guih_leader_slot_t * lslot = fd_guih_get_leader_slot( gui, _slot );
...
void
fd_guih_unbecame_leader( fd_guih_t *                gui,
                        ulong                     _slot,
...
void
fd_guih_microblock_execution_begin( fd_guih_t *   gui,
                                   long         tspub_ns,
                                   ulong        _slot,
                                   fd_txn_e_t * txns,
                                   ulong        txn_cnt,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    fd_guih_txn_t * txn_entry = gui->txs[ (pack_txn_idx + i)%FD_GUIH_TXN_HISTORY_SZ ];
    ...
    txn_entry->flags                      |= (uchar)FD_GUIH_TXN_FLAGS_STARTED;
    txn_entry->flags                      |= (uchar)fd_uint_if( !!(txn_payload->flags & FD_TXN_P_FLAGS_IS_SIMPLE_VOTE), FD_GUIH_TXN_FLAGS_IS_SIMPLE_VOTE, 0U );
    txn_entry->flags                      |= (uchar)fd_uint_if( (txn_payload->flags & FD_TXN_P_FLAGS_BUNDLE) || (txn_payload->flags & FD_TXN_P_FLAGS_INITIALIZER_BUNDLE), FD_GUIH_TXN_FLAGS_FROM_BUNDLE, 0U );
  }
...
void
fd_guih_microblock_execution_end( fd_guih_t *     gui,
                                 long           tspub_ns,
                                 ulong          bank_idx,
                                 ulong          _slot,
                                 ulong          txn_cnt,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    txn_entry->tips                      = tips;
    txn_entry->flags                    |= (uchar)FD_GUIH_TXN_FLAGS_ENDED;
    txn_entry->flags                    &= (uchar)(~(uchar)FD_GUIH_TXN_FLAGS_LANDED_IN_BLOCK);
    txn_entry->flags                    |= (uchar)fd_uint_if( !!(txn_p->flags & FD_TXN_P_FLAGS_EXECUTE_SUCCESS), FD_GUIH_TXN_FLAGS_LANDED_IN_BLOCK, 0U );
  }
