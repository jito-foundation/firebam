...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, FD_BANK_ABI_TXN_ALIGN, MAX_TXN_PER_MICROBLOCK*FD_BANK_ABI_TXN_FOOTPRINT_SIDECAR_MAX );
...
bank_tile_fill_bam_nonrevert_result( fd_bam_bundle_result_t * res,
  ...
  if( FD_UNLIKELY( !sanitize_success || !committed ) ) {
    fd_bam_result_mark_not_committed_txn_error( res, result_idx,
        sanitize_success ? fd_bam_txn_err_from_transaction_error_idx( transaction_err_idx ) :
...
handle_microblock( fd_bank_ctx_t *     ctx,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    sanitized_idx++;
    int transaction_err_idx = transaction_err[ sanitized_idx-1UL ];

    /* Stash the result in the flags value so that pack can inspect it. */
    txn->flags = (txn->flags & 0x00FFFFFFU) | ((uint)transaction_err_idx<<24);

    ctx->metrics.transaction_result[ transaction_err_idx ]++;
    ...
    for( ulong i=0UL; i<txn_cnt; i++ ) {
      ...
      if( FD_LIKELY( execution_success ) ) {
        ...
        fd_bam_result_add_txn_error( &res, i,
            sanitize_success ? fd_bam_txn_err_from_transaction_error_idx( transaction_err[ i ] ) :
                               bam_types_TransactionErrorReason_SANITIZE_FAILURE );
      }
    }
...
unprivileged_init( fd_topo_t const *      topo,
  ...
  ctx->txn_sidecar_mem = FD_SCRATCH_ALLOC_APPEND( l, FD_BANK_ABI_TXN_ALIGN, MAX_TXN_PER_MICROBLOCK*FD_BANK_ABI_TXN_FOOTPRINT_SIDECAR_MAX );
