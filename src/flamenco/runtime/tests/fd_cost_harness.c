...
fd_solfuzz_pb_cost_run( fd_solfuzz_runner_t * runner,
                        ...
                        void *                output_buf,
                        ulong                 output_bufsz ) {
  fd_exec_test_cost_context_t const * input  = fd_type_pun_const( input_ );
  fd_exec_test_cost_result_t **       output = fd_type_pun( output_ );
  ...
  /* Build output into the caller-provided scratch buffer */
  ulong output_end = (ulong)output_buf + output_bufsz;
  FD_SCRATCH_ALLOC_INIT( l, output_buf );
  fd_exec_test_cost_result_t * result =
    FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_exec_test_cost_result_t), sizeof(fd_exec_test_cost_result_t) );
  FD_TEST( _l <= output_end );
  *result = (fd_exec_test_cost_result_t)FD_EXEC_TEST_COST_RESULT_INIT_ZERO;

  result->has_cost = 1;
  if( txn_out->details.txn_cost.type==FD_TXN_COST_TYPE_TRANSACTION ) {
    result->signature_cost = txn_out->details.txn_cost.transaction.signature_cost;
    result->write_lock_cost = txn_out->details.txn_cost.transaction.write_lock_cost;
    result->data_bytes_cost = txn_out->details.txn_cost.transaction.data_bytes_cost;
    result->programs_execution_cost = txn_out->details.txn_cost.transaction.programs_execution_cost;
    result->loaded_accounts_data_size_cost = txn_out->details.txn_cost.transaction.loaded_accounts_data_size_cost;
    result->allocated_accounts_data_size = txn_out->details.txn_cost.transaction.allocated_accounts_data_size;
  } else {
    ...
    result->signature_cost = FD_PACK_COST_PER_SIGNATURE;
    result->write_lock_cost = FD_WRITE_LOCK_UNITS * 2UL;
    result->data_bytes_cost = 0UL;
    result->programs_execution_cost = 2100UL;
    result->loaded_accounts_data_size_cost = 8UL;
    result->allocated_accounts_data_size = 0UL;
  }
  ...
  result->total_cost = fd_ulong_sat_add( result->signature_cost, result->write_lock_cost );
  result->total_cost = fd_ulong_sat_add( result->total_cost, result->data_bytes_cost );
  result->total_cost = fd_ulong_sat_add( result->total_cost, result->programs_execution_cost );
  result->total_cost = fd_ulong_sat_add( result->total_cost, result->loaded_accounts_data_size_cost );
