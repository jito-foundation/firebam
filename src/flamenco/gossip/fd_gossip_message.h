...
/* fd_gossip_version_cstr converts gossip version fields to a null
   ...
   prerelease notation.
    ...
    Patch field semantics:
        - If prerelease_bits==0 (stable), `patch` is rendered as the
          normal semver patch component:
          `<major>.<minor_actual>.<patch>`.
        - If prerelease_bits!=0 (prerelease), the semver patch component
          is forced to 0 and `patch` is interpreted as the prerelease
          sequence number, rendered as:
          `<major>.<minor_actual>.0-<channel>.<patch>`.
...
fd_gossip_version_cstr( ushort major,
  ...
  if( FD_UNLIKELY( prerelease_bits ) ) {
    ...
    return fd_cstr_printf_check( out, out_sz, NULL, "%hu.%hu.0-%s.%hu", major, minor_actual, names[ prerelease_bits ], patch );
  } else {
