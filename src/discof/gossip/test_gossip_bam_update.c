...
setup_gossip( fd_gossip_tile_ctx_t * ctx,
              fd_gossip_contact_info_t * my_ci ) {
  ulong max_values      = 128UL;
  ulong entrypoints_len = 1UL;

  fd_ip4_port_t entrypoints[1] = {0};
  entrypoints[0].addr = my_ci->sockets[ FD_GOSSIP_CONTACT_INFO_SOCKET_GOSSIP ].ip4;
  entrypoints[0].port = my_ci->sockets[ FD_GOSSIP_CONTACT_INFO_SOCKET_GOSSIP ].port;
...
test_gossip_apply_bam_contact_updates_contact_info( void ) {
  ...
  for( uchar i=0; i<32; i++ ) ctx.identity_key->uc[ i ] = i;
  ...
  */
  ...
  base_ci.sockets[ FD_GOSSIP_CONTACT_INFO_SOCKET_GOSSIP ]            = (fd_gossip_socket_t){ .is_ipv6 = 0, .ip4 = default_gossip.addr,   .port = default_gossip.port   };
  base_ci.sockets[ FD_GOSSIP_CONTACT_INFO_SOCKET_TPU ]               = (fd_gossip_socket_t){ .is_ipv6 = 0, .ip4 = default_tpu.addr,      .port = default_tpu.port      };
  base_ci.sockets[ FD_GOSSIP_CONTACT_INFO_SOCKET_TPU_FORWARDS ]      = (fd_gossip_socket_t){ .is_ipv6 = 0, .ip4 = default_tpu.addr,      .port = default_tpu.port      };
  base_ci.sockets[ FD_GOSSIP_CONTACT_INFO_SOCKET_TPU_QUIC ]          = (fd_gossip_socket_t){ .is_ipv6 = 0, .ip4 = default_tpu_quic.addr, .port = default_tpu_quic.port };
  base_ci.sockets[ FD_GOSSIP_CONTACT_INFO_SOCKET_TPU_FORWARDS_QUIC ] = (fd_gossip_socket_t){ .is_ipv6 = 0, .ip4 = default_tpu_quic.addr, .port = default_tpu_quic.port };
