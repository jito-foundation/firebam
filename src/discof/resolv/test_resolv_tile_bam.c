...
test_harness_new( test_harness_t * h ) {
  ...
  h->mcaches[ 2 ]      = h->bam_mcache;
  h->depths [ 0 ]      = TEST_MCACHE_DEPTH;
  h->depths [ 1 ]      = TEST_MCACHE_DEPTH;
  h->depths [ 2 ]      = TEST_MCACHE_DEPTH;
  h->cr_avail[ 0 ]     = ULONG_MAX;
  h->cr_avail[ 1 ]     = ULONG_MAX;
  h->cr_avail[ 2 ]     = ULONG_MAX;
...
test_no_bank_deser_result( uchar batch_idx,
  ...
  FD_TEST( h->seqs[ 0 ]==0UL );
  FD_TEST( h->seqs[ 2 ]==expected_bam_publishes );
